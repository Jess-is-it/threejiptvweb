...
- Movies page now shows a friendly first-screen loader overlay with rotating status text while the hero, queue-backed rows, and first poster batch are warming up. It stays over the hero/top catalog bands until the first-screen artwork preload reaches a readiness threshold or times out, so cold refreshes no longer leave users staring at silent skeletons.
- `Movies -> All Movies` now preloads more aggressively for fast scrollers: the append observer triggers earlier, appended batches are larger, the page eagerly treats the first six rendered rows as high-priority artwork, and a background preload loop warms poster URLs for a much larger near-fold window. This is intended to reduce long skeleton gaps after users blow past the first visible rows.
- Hero section now uses backdrop-oriented artwork only again. Public catalog normalization no longer aliases poster art into the `backdrop` field, hero slide ordering prefers entries that actually have backdrops, and the shared hero renderer stays in wide `object-cover` backdrop mode instead of switching into a poster-contain layout.
- Search feels faster than Movies because it renders one straightforward results grid and lets the browser focus on the visible posters. Movies now borrows that priority model more closely: the first-screen preload favors the early `All Movies` cards sooner, while the heavier speculative background warmup is delayed until the `All Movies` section is actually in view and then only preloads the next rows ahead instead of replaying a large top-of-grid window.
- Hero slides now fetch TMDB details by `tmdbId`/`mediaType` when available instead of always doing slower title searches, and the hero renders a blurred poster placeholder while the wide backdrop is still loading so the section no longer sits on a black screen first.
