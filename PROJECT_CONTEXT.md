...
- Public Movies/Series catalogs now synchronously hydrate first-view TMDB artwork in the initial `/api/xuione/vod` and `/api/xuione/series` responses (with short server-side catalog caching for both), preconnect the browser to `image.tmdb.org`, and prioritize the hero/first-row images more aggressively so the top of the page stops painting text/empty placeholders long before artwork arrives.
...
- Category Settings are temporarily disabled in the live public runtime. `/api/public/settings` no longer exposes `settings.catalog`, so Movies/Series fall back to built-in catalog labels/layout/hero/category defaults, and the admin `Category Settings` route/nav is hidden by redirecting `/admin/category-settings` to `/admin/settings` without deleting the stored catalog config from the admin DB.
