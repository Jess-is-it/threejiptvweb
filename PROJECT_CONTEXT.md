...
1. **Public playback flow**
   ...
   - Live hero playback on `/live` uses `VideoPlayer` in muted-background mode; clicking **Play** enters fullscreen and swaps to the full player chrome with in-player `menuNavigation` (Categories/Channels browser + Previous/Next channel). Live channel switching swaps the source first inside the fullscreen click, delays parent hero-state sync slightly, keeps the media element's real/default muted state aligned so fullscreen channel changes do not fall back to the `Tap to unmute` overlay, hides the seek bar while fullscreen live playback is active, removes the fullscreen live play/pause button, wires the player back button to exit the live hero fullscreen instead of following browser history, and normalizes XUI pseudo-path channel logos like `s:1:/images/...` into proxied `/api/xuione/image` URLs that resolve relative XUI `/images/...` assets through the artwork cache.
