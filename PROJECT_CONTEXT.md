...
1. **Public playback flow**
   ...
   - Live hero playback on `/live` uses `VideoPlayer` in muted-background mode; clicking **Play** enters fullscreen and swaps to the full player chrome with in-player `menuNavigation` (Categories/Channels browser + Next channel). Live channel switching now swaps the source first inside the fullscreen click and delays parent hero-state sync slightly so the new stream can start with audio before the rerender.
