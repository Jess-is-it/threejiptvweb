...
- **Main domains:**
  - Public IPTV streaming UI (Home + Movies/Series/Live + immersive watch pages; `/` is the post-login Home page)
...
- Media Library admin:
  ...
  - Both list pages support **Add Subtitles** for already deployed NAS titles. The modal searches Movie/Series library rows with a `nasPath`, uploads one or more subtitle files directly to the deployed NAS folder, and renames them beside the matched video as `<video-basename>.<language>.<ext>` (default language `en`). Movie uploads target the single resolved movie video; Series uploads require subtitle filenames to include `SxxEyy` / `1x02` markers so each subtitle can be matched to the deployed episode file.
...
- AutoDownload admin:
  ...
  - `/admin/autodownload/storage` (Storage & Mount dashboard/settings; redirects expired admin API sessions back to `/admin/login`, refreshes saved SMB/CIFS values when Settings opens, and intentionally displays saved SMB username/password in the settings modal for current local-ops convenience)
...
- AutoDownload: `/api/admin/autodownload/*` (engine, mount, download client, settings, processing, scheduler, xui, logs)
 - Request admin APIs:
  ...
  - release diagnostics endpoint `GET /api/admin/autodownload/release/diagnostics` compares the Series processing folder (`.../qBittorrent/Series/<processing>`) against DB `downloadsSeries[].holdDir` paths to spot orphaned or missing hold directories
...
1. **Public playback flow**
   ...
   - Live hero playback on `/live` uses `VideoPlayer` in muted-background mode; clicking **Play** enters fullscreen and swaps to the full player chrome with in-player `menuNavigation` (Categories/Channels browser + Previous/Next channel). Live channel switching swaps the source first inside the fullscreen click, delays parent hero-state sync slightly, keeps the same fullscreen player instance across channel changes, and re-applies the media element's real/default unmuted state after source swaps so fullscreen channel changes do not fall back to the `Tap to unmute` overlay. The player hides the seek bar while fullscreen live playback is active, removes the fullscreen live play/pause button, wires the player back button to exit the live hero fullscreen instead of following browser history, treats common non-fatal live HLS buffer events (`bufferStalledError`, `bufferSeekOverHole`, `bufferNudgeOnStall`) as recoverable/noisy stream conditions with a throttled live-edge resume instead of console warnings, safely catches interrupted `play()` promises during source reloads, resets the internal source marker whenever an HLS instance is destroyed so metadata/server-list re-renders cannot leave a dead MediaSource blob black screen, and normalizes XUI pseudo-path channel logos like `s:1:/images/...` into proxied `/api/xuione/image` URLs that resolve relative XUI `/images/...` assets through the artwork cache.
...
- XUI playback/server selection uses `lib/server/xuiServerRouting.js`. Local/LAN requests should prefer configured public XUI hostnames and only use raw private IPs when no public hostname is configured; if split-horizon DNS resolves that hostname to a private address, keep the hostname/protocol intact so XUI Host/TLS rules still match while traffic remains local.
...
- Public Movies/Series kids filtering is now server-driven. The browser no longer scans TMDB item-by-item when Kids profile is selected; instead, catalog routes attach persistent `kidsSafe` metadata, login/header catalog warmers prefill that cache in the background, and Kids mode pages simply filter the already-tagged library. Upcoming/Leaving Soon public APIs also attach the same `kidsSafe` tags so those rows stay consistent.
...
- Processing recovery handles interrupted app restarts: if a queue row is stuck in `Processing` but its cleaned `Reldate...` hold folder already exists, the next processing pass marks it `Cleaned`, restores `holdDir`/`finalTargetDir`, logs the recovery, and clears the public upcoming cache so Worth-to-wait rows update.
...
- Release workflow recognizes both AutoDownload qB hold roots (`<mountDir>/qBittorrent/.../<Cleaned and Ready>`) and manual-upload hold roots (`<mountDir>/Manual Upload/.../<Cleaned and Ready>`) when releasing due items, repairing stale released rows, and pruning empty `Reldate...` folders. If a due manual-upload hold folder exists on disk but its DB queue row/log is missing, release adopts the folder by parsing the `Reldate.../<Title (Year)-...>` path, resolving TMDB, moving it into the final library, and creating a released queue/log record. If a previous release move already placed files in the final target but left the hold pointer stale/missing, the row is normalized as released instead of staying `failed_release`.
...
- Queue-backed upcoming movie cards now show a compact month/day release badge (`MMM D`); clicking those cards opens the preview modal directly (play-first bypass), and the modal’s top metadata line shows ratings first, then `Release date: <MMM D, YYYY>`, followed by genre/duration/year, plus description/cast and bottom-left `Remind me` + `More info` actions (red icon button via POST `/api/public/autodownload/upcoming`, action `remind`). Trailer previews opened from movie cards autoplay with audio on click instead of forcing muted playback, and the modal now includes an explicit top-right close button in addition to backdrop close.
...
- Movie detail pages now use icon-bearing hero action buttons, include a top-left Back button inside the hero, keep the hero title free of the auto-appended `(YYYY)` suffix, use the same page padding rhythm as `/movies`, load TMDB cast metadata for normal library titles including profile images/characters, dim cast photos with an overlay for readability, and replace the old placeholder `Related movies` block with a working `More like this` local-library row based on shared movie genres. The Watchlist hero action persists enriched movie metadata into `3jtv_watchlist_v1` so `/bookmarks` can render saved titles with details and direct playback, and it shows a visible add/remove success toast. The separate trailer CTA is intentionally hidden; when a YouTube trailer is available, the hero shows an upper-right countdown, then the backdrop auto-switches to a muted trailer after about 6 seconds, clips trailer overflow inside the hero boundary, covers the full hero frame with a 16:9 iframe crop, and shows a lower-right mute/unmute icon action that sends YouTube player commands without remounting/restarting the trailer. If the browser is offline or the YouTube iframe fails to load within the guarded timeout, the trailer is suppressed and the poster/backdrop remains visible instead of showing a black frame.
- Series detail pages mirror the movie-detail presentation: hero titles do not auto-append `(YYYY)`, the hero has a top-left Back button, TMDB-enriched poster/backdrop/overview/rating/genre/cast/trailer metadata, the same trailer countdown/autoplay/mute-unmute hero behavior with the same offline/iframe-load fallback, icon-bearing Watchlist/Remind actions with success toasts, cast cards with dimmed profile-image overlays, page padding matching `/series`, and a working local-library `More like this` row scored by shared series genres.
- Public `/bookmarks` is the functional My Watchlist page. It reads `3jtv_watchlist_v1` on mount, live-refreshes from same-tab watchlist events and cross-tab storage events, shows total/movie/series counts, supports All/Movies/Series filters, Recent/Title/Type sorting, per-title removal, clear-all, detail links, and movie quick-play that preserves the return path back to `/bookmarks`. Saved-title timestamps are shown as relative age labels like `Added 1d ago`; removing titles and clearing the list show visible success toasts.
...
- Admin AutoDownload includes `/admin/autodownload/sources` for Movie/Series Download Sources (YTS/TPB/Jackett/Prowlarr/EZTV depending on media type), with provider cards, status badges, active base display, test/test-active actions, configuration modal, and provider logs. Series mode displays TV-pack-only guidance and uses an IMDb TV test query.
...
- Released AutoDownload rows are terminal: sync, retry dispatch, manual retry, and timeout checks must not requeue them when their qB torrent disappears. Missing released torrents should only normalize the row (`status=Released`, `releaseState=released`, no stale error/retry metadata) and optionally record qB cleanup state.
...
- Movie and Series Selection Log run lists sort by actual `runAt` time descending. `updatedAt` and `releasedAt` are only fallbacks for legacy rows, so a release update cannot move an older run above the latest selection run.
...
- Header request CTA is now contextual by route (`Request` on Home, `Request Movie` on Movies pages, `Request Series` on Series pages) and routes to `/request?type=all|movie|tv`. The active public nav item uses the same brand color as the Play button on both desktop and mobile navigation.
...
- Selection-specific download gates and selection strategies are edited from the selection pages, not from `AutoDownload Settings -> Limits & Timeouts`: `Movie Selection Log Settings` owns `maxMovieGb`, `minMovieSeeders`, and `movieSelectionStrategy`; `Series Selection Log Settings` is now TV-pack-only and owns `strictSeriesReplacement`, `deletePartialSeriesOnReplacementFailure`, one visible `TV Packs Only` policy (`newSeries`), min seeders, max TV pack size, timeout, and strategy counts. The admin settings route supports `PATCH` with `section: "selection_rules"` for these page-local modals, and the Series Selection Log page/legacy alias route must render the same title/actions as Movies (`Series Selection Log`, `Trigger AutoDownload Once`, `Clear Log`).
- Series AutoDownload no longer exposes or runs episode bootstrap / existing-series continuation pipelines. `newSeriesEpisode` and `existingSeries` remain in the settings schema only for legacy rows and are normalized disabled. `deferredRetry` remains internal for timeout/replacement-triggered runs but is also TV-pack-only.
- The Series Selection Log table is still pipeline-aware for legacy history, but normal controls should show only `TV Packs`, `TV Pack Retry`, and `Legacy` filters. New runs must classify selected rows as Season 1 TV packs, not first/next episodes.
- Series source selection and dispatch must enforce full Season 1 TV packs at both points: selection filters provider candidates by pack naming, seeders, usable link/hash, and max TV pack size before queueing; dispatch revalidates stale/preselected sources and fallback searches so episode-only results can never start in qBittorrent.
...
- Series source validation now accepts provider IMDb-id matches from Jackett/Prowlarr/EZTV in addition to normalized title matching, and series selection must skip a rejected candidate source instead of aborting the whole run and writing an empty orphan Series Selection Log row.
- TMDB-backed AutoDownload selection/detail fetches now retry transient TMDB responses (`429/500/502/503/504`) before failing, reducing random empty selection-log runs caused by one-off TMDB upstream errors.
