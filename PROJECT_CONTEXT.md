...
- Selection-specific download gates and selection strategies are now edited from the selection pages, not from `AutoDownload Settings -> Limits & Timeouts`: `Movie Selection Log Settings` owns `maxMovieGb`, `minMovieSeeders`, and `movieSelectionStrategy`; `Series Selection Log Settings` owns `strictSeriesReplacement`, `deletePartialSeriesOnReplacementFailure`, and independent series pipelines (`newSeries`, `newSeriesEpisode`, `existingSeries`, `deferredRetry`) with their own enabled flag, selection strategy, min seeders, relevant max size, and timeout. The admin settings route supports a `PATCH` with `section: "selection_rules"` for these page-local modals, and the Series Selection Log page/legacy alias route must render the same title/actions as Movies (`Series Selection Log`, `Trigger AutoDownload Once`, `Clear Log`).
- `Series Selection Log Settings` is tabbed: `Overview`, `New S1 Pack`, `New Episode`, `Existing Series`, and `Retry`. Keep general replacement cleanup settings in Overview and pipeline-specific source gates/strategy inside each pipeline tab with explanatory notes. Show max season size only for season-pack pipelines, and max episode size only for episode pipelines.
- The Series Selection Log table should be pipeline-aware: the run list shows pipeline counts instead of only Recent/Classic bucket columns, and the jobs modal exposes each selected row's pipeline/mode with a pipeline filter including `New S1 Pack` and `New Episode`.
- Series pipeline behavior is intentionally separate: `newSeries` bootstraps missing shows with a healthy Season 1 pack only, `newSeriesEpisode` bootstraps missing shows with one first aired Season 1 episode only, `existingSeries` is disabled by default and continues shows already present in the NAS by selecting the next missing aired episode, and `deferredRetry` is used for timeout/replacement-triggered selection with stricter source gates.
