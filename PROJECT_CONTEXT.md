...
- Hero backdrop rendering now distinguishes between true wide backdrops and poster-style fallback art. When a slide only has poster/fallback artwork, the hero uses a contained foreground image with a blurred fill layer behind it instead of force-covering the full hero area, so faces/heads are not cropped as harshly.
