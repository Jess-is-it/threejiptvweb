...
export function buildAutodownloadReadinessModel({
  ...
  const items = [
    ...
    },
    {
      ...
      detail: `Min seeders M/S: ${asNumber(sourceFilters?.minMovieSeeders, 0)}/${asNumber(sourceFilters?.minSeriesSeeders, 0)} • Size caps M/E: ${sizeLimits?.maxMovieGb ?? '—'}GB/${sizeLimits?.maxEpisodeGb ?? '—'}GB`,
      href: '/admin/autodownload/settings',
      required: true,
