...
export function defaultNotificationSettings() {
  return {
    telegram: {
      ...
      nasStatusEnabled: false,
      vpnStatusEnabled: false,
      systemHttpsStatusEnabled: false,
      xuiHttpsStatusEnabled: false,
...
export function normalizeNotificationSettings(input = {}) {
  ...
  return {
    telegram: {
      ...
      nasStatusEnabled: normalizeBoolean(telegram.nasStatusEnabled),
      vpnStatusEnabled: normalizeBoolean(telegram.vpnStatusEnabled),
      systemHttpsStatusEnabled: normalizeBoolean(telegram.systemHttpsStatusEnabled),
      xuiHttpsStatusEnabled: normalizeBoolean(telegram.xuiHttpsStatusEnabled),
...
function normalizeServiceStatusItem(row = {}) {
  const source = row && typeof row === 'object' ? row : {};
  if (source.isUp !== true && source.isUp !== false) return null;
  const checkedAt = normalizeMaybeNumber(source.checkedAt, { min: 1, max: 9999999999999 });
  const lastChangedAt = normalizeMaybeNumber(source.lastChangedAt, { min: 1, max: 9999999999999 });
  const lastSentAt = normalizeMaybeNumber(source.lastSentAt, { min: 1, max: 9999999999999 });
  return {
    isUp: source.isUp === true,
...
export function normalizeNotificationState(input = {}) {
  ...
  const serviceStatus = telegram.serviceStatus && typeof telegram.serviceStatus === 'object' ? telegram.serviceStatus : {};
  ...
  const serviceStatusLastCheckedAtRaw = Number(serviceStatus.lastCheckedAt || 0);
  ...
  const serviceStatusSource = serviceStatus.services && typeof serviceStatus.services === 'object' ? serviceStatus.services : {};
  const services = {};
  for (const [key, row] of Object.entries(serviceStatusSource)) {
    const serviceKey = String(key || '').trim();
    if (!/^(nas|vpn|systemHttps|xuiHttps)$/.test(serviceKey)) continue;
    const item = normalizeServiceStatusItem(row);
    if (!item) continue;
    services[serviceKey] = item;
  }
  ...
  return {
    telegram: {
      ...
      serviceStatus: {
        lastCheckedAt:
          Number.isFinite(serviceStatusLastCheckedAtRaw) && serviceStatusLastCheckedAtRaw > 0
            ? serviceStatusLastCheckedAtRaw
            : null,
        services,
      },
    },
