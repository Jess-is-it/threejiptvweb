...
import { saveOpenSubtitleToLocalLibrary } from './localSubtitleService';
...
function buildTrackUrl({ fileId, lang, label, cacheTarget = null }) {
  ...
  if (cacheTarget?.mediaType) u.searchParams.set('mediaType', String(cacheTarget.mediaType));
  if (cacheTarget?.title) u.searchParams.set('title', String(cacheTarget.title));
  if (cacheTarget?.year) u.searchParams.set('year', String(cacheTarget.year));
  if (cacheTarget?.seasonNumber) u.searchParams.set('seasonNumber', String(cacheTarget.seasonNumber));
  if (cacheTarget?.episodeNumber) u.searchParams.set('episodeNumber', String(cacheTarget.episodeNumber));
  ...
  languages.forEach((langCode) => {
    ...
    selected.push({
      ...
      url: buildTrackUrl({
        ...
        lang: best.langCode,
        label: best.label,
        cacheTarget: { mediaType: 'movie', title: rawTitle, year },
      }),
    });
  });
...
export async function getSubtitleFileAsVtt({ fileId, cacheTarget = null } = {}) {
  ...
  if (cacheTarget?.title) {
    await saveOpenSubtitleToLocalLibrary({
      ...cacheTarget,
      subtitleText: text,
    }).catch(() => null);
  }
