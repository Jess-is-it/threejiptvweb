...
import { getQbittorrentVpnSettingsSafe } from './autodownload/vpnService';
import { xuiApiCall } from './autodownload/xuiService';
import { getAdminDb } from './adminDb';
import { getPublicHttpsStatus } from './publicHttps';
import { getSecret, secretKeys } from './secrets';
import { getXuiHttpsStatus } from './xuiHttps';
...
function describeEnabledAreas(settings) {
  ...
  if (settings?.telegram?.nasStatusEnabled) areas.push('NAS Up/down');
  if (settings?.telegram?.vpnStatusEnabled) areas.push('VPN Up/down');
  if (settings?.telegram?.systemHttpsStatusEnabled) areas.push('3JTV HTTPS Up/down');
  if (settings?.telegram?.xuiHttpsStatusEnabled) areas.push('XUI HTTPS Up/down');
  return areas;
}
...
function serviceStatusMonitors(settings = null) {
  const telegram = settings?.telegram || {};
  return [
    {
      key: 'nas',
      label: 'NAS',
      enabled: telegram.nasStatusEnabled === true,
    },
    {
      key: 'vpn',
      label: 'VPN',
      enabled: telegram.vpnStatusEnabled === true,
    },
    {
      key: 'systemHttps',
      label: '3JTV HTTPS',
      enabled: telegram.systemHttpsStatusEnabled === true,
    },
    {
      key: 'xuiHttps',
      label: 'XUI HTTPS',
      enabled: telegram.xuiHttpsStatusEnabled === true,
    },
  ];
}

function normalizeServiceStatus({ key = '', label = '', isUp = false, summary = '', checkedAt = null } = {}) {
  return {
    key: String(key || '').trim(),
    label: String(label || key || '').trim(),
    isUp: isUp === true,
    summary: compactOneLine(summary).slice(0, 600),
    checkedAt: Number(checkedAt || now()) || now(),
  };
}

async function resolveNasServiceStatus({ mountStatus = null } = {}) {
  let status = mountStatus;
  if (!status || typeof status !== 'object') {
    status = await fetchMountStatus().catch((error) => ({ ok: false, writable: false, error: error?.message || 'NAS status check failed.' }));
  }
  const isUp = status?.ok === true && status?.writable === true;
  const path = String(status?.path || status?.mountDir || '').trim();
  const summary = isUp
    ? `Mounted and writable${path ? ` at ${path}` : ''}.`
    : String(status?.error || 'NAS is offline or not writable.');
  return normalizeServiceStatus({ key: 'nas', label: 'NAS', isUp, summary });
}

async function resolveVpnServiceStatus() {
  const vpn = await getQbittorrentVpnSettingsSafe().catch((error) => ({
    enabled: true,
    lastTestOk: false,
    lastAppliedOk: false,
    lastError: error?.message || 'VPN status check failed.',
  }));
  const enabled = vpn?.enabled === true;
  const hasIssue = enabled && (vpn?.lastTestOk === false || vpn?.lastAppliedOk === false);
  const healthy = enabled && !hasIssue && (vpn?.lastTestOk === true || vpn?.lastAppliedOk === true);
  ...
  return normalizeServiceStatus({ key: 'vpn', label: 'VPN', isUp: healthy, summary });
}

async function resolveSystemHttpsServiceStatus() {
  const status = await getPublicHttpsStatus().catch((error) => ({
    cloudflaredRunning: false,
    publicServiceCheck: { status: 'FAILED', error: error?.message || '3JTV HTTPS status check failed.' },
    settings: {},
  }));
  const running = status?.cloudflaredRunning === true;
  const publicOk = checkSucceeded(status?.publicServiceCheck);
  const publicUrl = String(status?.settings?.publicUrl || '').trim();
  const summary = running && publicOk
    ? `Cloudflare connector is running and ${publicUrl || 'public URL'} is reachable.`
    : !running
      ? 'Cloudflare connector is not running.'
      : String(status?.publicServiceCheck?.error || `Public URL check failed (${status?.publicServiceCheck?.status || 'unknown'}).`);
  return normalizeServiceStatus({ key: 'systemHttps', label: '3JTV HTTPS', isUp: running && publicOk, summary });
}

async function resolveXuiHttpsServiceStatus() {
  const status = await getXuiHttpsStatus().catch((error) => ({
    lastRemoteStatus: { serviceActive: 'failed', error: error?.message || 'XUI HTTPS status check failed.' },
    publicServiceCheck: { status: 'FAILED', error: error?.message || 'XUI HTTPS status check failed.' },
    settings: {},
  }));
  const remoteState = String(status?.lastRemoteStatus?.serviceActive || '').trim();
  const remoteActive = !remoteState || remoteState === 'active';
  const publicOk = checkSucceeded(status?.publicServiceCheck);
  const publicUrl = String(status?.settings?.publicUrl || '').trim();
  const summary = remoteActive && publicOk
    ? `Remote connector is active and ${publicUrl || 'public XUI URL'} is reachable.`
    : remoteState && remoteState !== 'active'
      ? `Remote connector is ${remoteState}.`
      : String(status?.publicServiceCheck?.error || `Public XUI URL check failed (${status?.publicServiceCheck?.status || 'unknown'}).`);
  return normalizeServiceStatus({ key: 'xuiHttps', label: 'XUI HTTPS', isUp: remoteActive && publicOk, summary });
}

async function resolveServiceStatusSnapshot({ settings = null, mountStatus = null } = {}) {
  const monitors = serviceStatusMonitors(settings).filter((item) => item.enabled);
  const results = {};
  for (const monitor of monitors) {
    try {
      if (monitor.key === 'nas') results[monitor.key] = await resolveNasServiceStatus({ mountStatus });
      else if (monitor.key === 'vpn') results[monitor.key] = await resolveVpnServiceStatus();
      else if (monitor.key === 'systemHttps') results[monitor.key] = await resolveSystemHttpsServiceStatus();
      else if (monitor.key === 'xuiHttps') results[monitor.key] = await resolveXuiHttpsServiceStatus();
    } catch (error) {
      results[monitor.key] = normalizeServiceStatus({
        key: monitor.key,
        label: monitor.label,
        isUp: false,
        summary: error?.message || `${monitor.label} status check failed.`,
      });
    }
  }
  return results;
...
function buildServiceStatusChangeMessage({ current, previous = null, timeZone = 'Asia/Manila' } = {}) {
  const up = current?.isUp === true;
  const direction = up ? 'UP' : 'DOWN';
  const emoji = up ? '🟢' : '🔴';
  const previousLabel = previous ? (previous.isUp ? 'UP' : 'DOWN') : 'UNKNOWN';
  const lines = [
    `${emoji} ${current?.label || 'Service'} is ${direction}`,
    `Changed: ${formatDateTimeInTimeZone(now(), timeZone)}`,
    `Previous: ${previousLabel}`,
  ];
  const summary = compactOneLine(current?.summary || '');
  if (summary) lines.push(`Details: ${summary}`);
  return normalizeMessage(lines.join('\n'));
}

async function maybeSendServiceStatusNotifications({ settings, override = null, mountStatus = null } = {}) {
  const monitors = serviceStatusMonitors(settings).filter((item) => item.enabled);
  if (!monitors.length) return { ok: true, skipped: true, reason: 'disabled' };

  const timeZone = await getNotificationTimeZone();
  const state = await getNotificationState();
  const previousServices = state?.telegram?.serviceStatus?.services || {};
  const snapshot = await resolveServiceStatusSnapshot({ settings, mountStatus });
  const nextServices = { ...previousServices };
  const changed = [];
  const checkedAt = now();

  for (const monitor of monitors) {
    const current = snapshot[monitor.key];
    if (!current) continue;
    const previous = previousServices?.[monitor.key] || null;
    const changedState = previous && previous.isUp !== current.isUp;
    const next = {
      isUp: current.isUp,
      label: current.label,
      summary: current.summary,
      checkedAt,
      lastChangedAt: changedState ? checkedAt : Number(previous?.lastChangedAt || checkedAt) || checkedAt,
      lastSentAt: Number(previous?.lastSentAt || 0) || null,
    };

    if (changedState) {
      await sendTelegramMessage({
        text: buildServiceStatusChangeMessage({ current, previous, timeZone }),
        override,
      });
      next.lastSentAt = checkedAt;
      changed.push({ key: monitor.key, label: current.label, isUp: current.isUp });
    }

    nextServices[monitor.key] = next;
  }
  ...
  return {
    ...
    changed,
    checked: Object.keys(snapshot).length,
  };
}
...
export async function runTelegramNotificationTick({ mountStatus = null, vodState = null } = {}) {
  ...
  const result = {
    ...
    serviceStatus: { ok: true, skipped: true, reason: 'disabled' },
  ...
  try {
    result.serviceStatus = await maybeSendServiceStatusNotifications({
      settings,
      override,
      mountStatus,
    });
  } catch (error) {
    result.serviceStatus = { ok: false, error: error?.message || 'Service status notification failed.' };
  }
