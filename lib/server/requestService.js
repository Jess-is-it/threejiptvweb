...
function buildRequestDetailLabel(mediaType, requestScope, seasonNumber, episodeNumber, episodeNumbers = []) {
...
function normalizeRequestTarget(raw, mediaType, { strict = false } = {}) {
  ...
  if (scope === 'season') {
    ...
    const episodeNumbers = episodeNumbersInput;
    const requestUnits = episodeNumbers.length || requestUnitsInput || 1;
    const requestDetailLabel = detailLabelInput || buildRequestDetailLabel(mt, 'season', seasonNumber, null, episodeNumbers);
    return {
      requestScope: 'season',
      seasonNumber,
      episodeNumber: null,
  ...
  if (scope === 'episode') {
    ...
    const requestDetailLabel = detailLabelInput || buildRequestDetailLabel(mt, 'episode', seasonNumber, episodeNumber, episodeNumbers);
    return {
      requestScope: 'episode',
      seasonNumber,
      episodeNumber,
...
export async function submitRequests({ username, items = [] } = {}) {
  ...
  for (const item of entries) {
    ...
    const row = {
      ...
      episodeNumber: item.episodeNumber,
      episodeNumbers: normalizeEpisodeNumbers(item.episodeNumbers),
      requestDetailLabel: trimDetailLabel(item.requestDetailLabel, buildRequestDetailLabel(item.mediaType, item.requestScope, item.seasonNumber, item.episodeNumber, item.episodeNumbers)),
      requestUnits,
