...
import { getCachedXuiMediaIndex, getXuiMediaIndex } from './xuiMediaIndexService';
...
function makeCatalogData({ movieExact, movieTitleOnly, movieTmdbIds, seriesExact, seriesTitleOnly, seriesTmdbIds, fetchedAt, source = 'unknown' } = {}) {
  return {
    movieExact: movieExact || new Set(),
    movieTitleOnly: movieTitleOnly || new Set(),
    movieTmdbIds: movieTmdbIds || new Set(),
    seriesExact: seriesExact || new Set(),
    seriesTitleOnly: seriesTitleOnly || new Set(),
    seriesTmdbIds: seriesTmdbIds || new Set(),
    fetchedAt: Number(fetchedAt || 0) || now(),
    source,
  };
}

function buildCatalogFromMediaIndex(index, source = 'xui_index_cache') {
  ...
  for (const row of Array.isArray(index?.movies) ? index.movies : []) {
  ...
  for (const row of Array.isArray(index?.series) ? index.series : []) {
...
export async function getXuiLibraryCatalog({ maxAgeMs = DEFAULT_CACHE_MS, force = false } = {}) {
  ...
  const cachedIndex = getCachedXuiMediaIndex({ maxAgeMs });
  if (cachedIndex) {
    const data = buildCatalogFromMediaIndex(cachedIndex);
    catalogCache = { at: now, data };
    return data;
  }

  const freshIndex = await getXuiMediaIndex({ maxAgeMs, force });
  const data = buildCatalogFromMediaIndex(freshIndex, 'xui_index_fresh');
  catalogCache = { at: now, data };
  return data;
}
