...
export async function ensureQbittorrentVpnReadyForDispatch() {
  ...
  try {
    ...
    if (!runtime.ok && recentDownloadOk) {
      relaxedOk = true;
    }
    ...
    const saved = await saveVpnConfig({
      ...
      vpn: {
        ...
        lastTestSummary: runtime.ok ? runtime.summary : relaxedOk ? `${runtime.summary} (relaxed: recent VPN download test ok)` : runtime.summary,
    ...
    return {
      ...
      required: true,
      summary: runtime.ok ? runtime.summary : relaxedOk ? `${runtime.summary} (relaxed: recent VPN download test ok)` : runtime.summary,
      runtime,
      vpn: toSafeVpn(saved),
    };
  } catch (e) {
