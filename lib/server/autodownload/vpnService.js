...
export async function ensureQbittorrentVpnReadyForDispatch() {
  ...
  try {
    ...
    // Some installations route qBittorrent traffic over VPN without the expected WireGuard interface/rules.
    // If the operator has recently validated VPN-only downloads, allow dispatch to proceed while still
    // recording the failing runtime checks for visibility.
    let relaxedOk = false;
    const lastDownloadOk = vpn.lastDownloadTestOk === true;
    const lastDownloadAt = Number(vpn.lastDownloadTestAt || 0) || 0;
    const recentDownloadOk = lastDownloadOk && lastDownloadAt > 0 && now() - lastDownloadAt < 7 * 24 * 60 * 60 * 1000;
    const lastAppliedOk = vpn.lastAppliedOk === true;
    const lastAppliedAt = Number(vpn.lastAppliedAt || 0) || 0;
    const recentApplyOk = lastAppliedOk && lastAppliedAt > 0 && now() - lastAppliedAt < 30 * 24 * 60 * 60 * 1000;
    const relaxedReason = recentDownloadOk ? 'recent VPN download test ok' : recentApplyOk ? 'VPN last applied ok' : '';
    if (!runtime.ok && (recentDownloadOk || recentApplyOk)) relaxedOk = true;
    ...
    const saved = await saveVpnConfig({
      ...
      vpn: {
        ...vpn,
        lastTestAt: now(),
        lastTestOk: runtime.ok || relaxedOk,
        lastTestSummary: runtime.ok ? runtime.summary : relaxedOk ? `${runtime.summary} (relaxed: ${relaxedReason})` : runtime.summary,
        lastError: runtime.ok || relaxedOk ? '' : runtime.summary,
      },
    });
    return {
      ok: runtime.ok || relaxedOk,
      required: true,
      summary: runtime.ok ? runtime.summary : relaxedOk ? `${runtime.summary} (relaxed: ${relaxedReason})` : runtime.summary,
      runtime,
      vpn: toSafeVpn(saved),
    };
  } catch (e) {
