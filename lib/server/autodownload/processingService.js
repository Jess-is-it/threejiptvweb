...
async function clearSelectionLogErrorState({ selectionLogId, type } = {}) {
  const logId = String(selectionLogId || '').trim();
  if (!logId) return false;

  const db = await getAdminDb();
  db.selectionLogs = Array.isArray(db.selectionLogs) ? db.selectionLogs : [];
  const idx = db.selectionLogs.findIndex((row) => String(row?.id || '').trim() === logId);
  ...
  if (next?.replacementFailure && typeof next.replacementFailure === 'object') {
    const replacementFailure = { ...next.replacementFailure };
    const selectionType = sanitizeType(type);
    if (Object.prototype.hasOwnProperty.call(replacementFailure, selectionType)) {
      delete replacementFailure[selectionType];
    }
    next.replacementFailure = Object.keys(replacementFailure).length ? replacementFailure : null;
  }

  const changed =
    String(prev?.errorMessage || '').trim() ||
    JSON.stringify(prev?.replacementFailure || null) !== JSON.stringify(next?.replacementFailure || null);
  if (!changed) return false;

  db.selectionLogs[idx] = next;
...
const PROCESSING_RECOVERY_MIN_AGE_MS = 10 * 60 * 1000;
...
async function cleanupManualUploadFailureResidue({ ssh, type, mountDir, settings, targets = [] } = {}) {
  const manualPaths = buildManualUploadPaths({ mountDir, type, settings });
  const allowedRoots = uniquePaths([manualPaths?.incomingDir || '', manualPaths?.processingDir || '']);
  const result = {
    ok: true,
    attempted: 0,
    removed: 0,
  ...
  for (const target of uniquePaths(targets)) {
    ...
    try {
      ...
      result.items.push({
        ...
        error: error?.message || 'Failed to clean manual upload residue.',
      });
    }
  }
...
function tmdbFromRecord(rec, type) {
  const t = sanitizeType(type);
  const tmdb = rec?.tmdb && typeof rec.tmdb === 'object' ? rec.tmdb : {};
  return {
    ok: true,
    id: Number(tmdb?.id || 0) || 0,
    mediaType: String(tmdb?.mediaType || (t === 'series' ? 'tv' : 'movie')).trim() || (t === 'series' ? 'tv' : 'movie'),
    title: String(tmdb?.title || rec?.title || '').trim(),
    year: String(tmdb?.year || rec?.year || '').trim(),
    ...
    posterPath: String(tmdb?.posterPath || tmdb?.poster_path || '').trim(),
    backdropPath: String(tmdb?.backdropPath || tmdb?.backdrop_path || '').trim(),
    originalLanguage: String(tmdb?.originalLanguage || '').trim(),
  };
}

function expectedHoldFolderLabel({ type, rec, tmdb, settings, list = [] } = {}) {
  const t = sanitizeType(type);
  const cleaningTemplates = getCleaningTemplates(settings);
  const qualityMeta = detectQualityMeta(rec);
  const staticTemplateTokens = {
    title: normalizeTemplateTokenValue(tmdb?.title || rec?.title || ''),
    year: normalizeTemplateTokenValue(tmdb?.year || rec?.year || ''),
  ...
  const folderFallback =
    t === 'series'
      ? `${tmdb?.title || rec?.title || 'Untitled'}${tmdb?.year || rec?.year ? ` (${tmdb?.year || rec?.year})` : ''}`
      : `${tmdb?.title || rec?.title || 'Untitled'}${tmdb?.year || rec?.year ? ` (${tmdb?.year || rec?.year})` : ''}-${qualityMeta.quality || 'HD'}`;
  return dedupeFolderLabel(applyNameTemplate(folderTemplate, staticTemplateTokens, { fallback: folderFallback }), {
    list,
...
function pickRecoveredHoldDir({ children = [], expectedLabel = '', tmdb = null } = {}) {
  const rows = Array.isArray(children) ? children : [];
  if (!rows.length) return '';
  const expectedNorm = normalizeMatchText(expectedLabel);
  if (expectedNorm) {
    const exact = rows.find((pathValue) => normalizeMatchText(basenamePath(pathValue)) === expectedNorm);
    if (exact) return exact;
  }

  const titleNorm = normalizeMatchText(tmdb?.title || '');
  const year = String(tmdb?.year || '').trim();
  const scored = rows
    .map((pathValue) => {
      const base = basenamePath(pathValue);
      const norm = normalizeMatchText(base);
      let score = 0;
      if (titleNorm && norm.includes(titleNorm)) score += 10;
      if (year && base.includes(year)) score += 5;
      if (expectedNorm && norm.startsWith(expectedNorm)) score += 3;
      return { path: pathValue, score };
    })
    .sort((a, b) => b.score - a.score);
  if (scored[0]?.score >= 10) return scored[0].path;
  return rows.length === 1 ? rows[0] : '';
}

async function recoverStaleProcessingRowsForType({ ssh, mountDir, settings, type, list = [] } = {}) {
  const t = sanitizeType(type);
  const rows = Array.isArray(list) ? list : [];
  const nextRows = [...rows];
  const result = {
  ...
  for (let index = 0; index < rows.length; index += 1) {
    ...
    const startedAt = Number(rec?.processingStartedAt || 0) || 0;
    if (!startedAt || now() - startedAt < PROCESSING_RECOVERY_MIN_AGE_MS) {
      result.skipped += 1;
      continue;
    }
    if (rec?.cleanedAt || rec?.holdDir) {
      result.skipped += 1;
      continue;
    }

    const tmdb = tmdbFromRecord(rec, t);
    if (!tmdb.title) {
      result.skipped += 1;
      continue;
    }
    const names = buildFinalPaths({ type: t, mountDir, tmdb, rec, settings });
    const releaseTag = String(rec?.releaseTag || '').trim() || buildReleaseTagFromDateKey(String(rec?.releaseDate || '').trim());
    if (!names?.processingDir || !releaseTag) {
      result.skipped += 1;
      continue;
    }

    const releaseDir = pathJoin(names.processingDir, releaseTag);
    const expectedLabel = expectedHoldFolderLabel({ type: t, rec, tmdb, settings, list: rows });
    const children = await listDirectChildrenNewestFirst(ssh, releaseDir).catch(() => []);
    const holdDir = pickRecoveredHoldDir({ children, expectedLabel, tmdb });
    if (!holdDir) {
      result.skipped += 1;
      continue;
    }

    const folderLabel = basenamePath(holdDir);
    const finalTargetDir = t === 'movie' ? `${names.finalDir}/${folderLabel}` : names.finalDir;
    const recoveredAt = now();
    nextRows[index] = {
      ...rec,
      status: 'Cleaned',
      cleanedAt: recoveredAt,
      tmdb: {
        ...(rec?.tmdb && typeof rec.tmdb === 'object' ? rec.tmdb : {}),
        id: tmdb.id,
      ...
      finalTargetDir,
      finalDir: holdDir,
      finalVideo: rec?.finalVideo || null,
      error: '',
      processingRecoveredAt: recoveredAt,
      processingRecoveryReason: 'Recovered cleaned hold folder after interrupted processing run.',
    };
    result.recovered += 1;
    result.changed += 1;
    result.logs.push({
      ...
      status: 'ok',
      summary: {
        recoveredCleanedHold: true,
        reason: 'interrupted_processing_db_write',
        expectedLabel,
      },
    });
  }
...
function buildFinalPaths({ type, mountDir, tmdb, rec, settings }) {
  ...
  const paths = rec?.manualUpload === true ? buildManualUploadPaths({ mountDir: baseRoot, type: t, settings }) : buildLibraryPaths({ mountDir: baseRoot, type: t, settings });
  const processingDir = String(paths?.processingDir || '').trim();
  if (t === 'series') {
...
function processingScript({
  ...
  languagePatterns,
  allowUnknownSubtitleLanguage = false,
  holdMode = false,
}) {
  ...
  const lines = [
    ...
    `ALLOW_UNKNOWN_SUB_LANG=${allowUnknownSubtitleLanguage ? '1' : '0'}`,
    '',
    'cleanup_stage() {',
    ...
    '  if [ -z "$lang" ]; then',
    '    [ "$ALLOW_UNKNOWN_SUB_LANG" = "1" ] && return 0',
    '    return 1',
    '  fi',
    ...
    '    else',
    '      subbase="$(render_template_name "$BASE_NAME" "" "" "" "$BASE_NAME")"',
    '    fi',
    ...
    '      ss="$(printf \'%02d\' "$((10#$season))")"',
    '      ee="$(printf \'%02d\' "$((10#$episode))")"',
...
export async function processOneCompleted({ type, id }) {
  ...
  const hasManualPath = Boolean(String(rec?.downloadPath || '').trim());
  if (!rec?.qbHash && !recoverableFailed && !hasManualPath) throw new Error('Missing qBittorrent hash.');
  if (statusLower !== 'completed' && !recoverableFailed) throw new Error('Only Completed items can be processed.');
  const isManualUpload = rec?.manualUpload === true || String(rec?.source?.provider || '').trim().toLowerCase() === 'manual_upload';
  ...
  try {
    ...
    let tmdb = null;
    if (rec?.manualUpload === true && rec?.tmdb?.title) {
      tmdb = {
        ok: true,
        id: Number(rec?.tmdb?.id || 0) || 0,
        mediaType: String(rec?.tmdb?.mediaType || (t === 'series' ? 'tv' : 'movie')).trim() || (t === 'series' ? 'tv' : 'movie'),
        title: String(rec?.tmdb?.title || '').trim(),
        year: String(rec?.tmdb?.year || rec?.year || '').trim(),
        releaseDate: String(rec?.tmdb?.releaseDate || '').trim(),
        genres: Array.isArray(rec?.tmdb?.genres) ? rec.tmdb.genres : [],
        rating: rec?.tmdb?.rating ?? null,
        runtime: rec?.tmdb?.runtime ?? null,
        overview: String(rec?.tmdb?.overview || '').trim(),
        posterPath: String(rec?.tmdb?.posterPath || rec?.tmdb?.poster_path || '').trim(),
        backdropPath: String(rec?.tmdb?.backdropPath || rec?.tmdb?.backdrop_path || '').trim(),
      };
    } else {
      const seed = rec.title || rec.qbName || props?.name || '';
      const guess = guessTitleYear(seed);
      tmdb = await resolveTmdbTitle({ kind: t, title: guess.title, year: guess.year });
    ...
    if (isManualUpload) {
      manualFailureTargets = uniquePaths([downloadPath, stageDir, holdDir]);
    }
    const finalTargetDir = t === 'movie' ? `${names.finalDir}/${folderLabel}` : names.finalDir;
    ...
    const script = processingScript({
      ...
      allowUnknownSubtitleLanguage: isManualUpload,
      holdMode: true,
    });
    ...
    const manualPaths = rec?.manualUpload === true ? buildManualUploadPaths({ mountDir: mount.mountDir, type: t, settings }) : null;
    const sourceCleanupRoots = uniquePaths([
      pathsForType.downloadingDir,
      pathsForType.downloadedDir,
      manualPaths?.incomingDir || '',
    ]);
    let sourceCleanup = {
      ok: false,
      removed: false,
    ...
    if (String(rec?.selectionLogId || '').trim()) {
      await clearSelectionLogErrorState({
        selectionLogId: rec.selectionLogId,
        type: t,
      }).catch(() => null);
    }
    ...
    const manualCleanup =
      isManualUpload && mount?.mountDir
        ? await cleanupManualUploadFailureResidue({
            ssh,
            type: t,
            mountDir: mount.mountDir,
            settings,
            targets: uniquePaths([rec?.downloadPath || '', ...(Array.isArray(manualFailureTargets) ? manualFailureTargets : [])]),
          }).catch((cleanupError) => ({
            ok: false,
            attempted: uniquePaths([rec?.downloadPath || '', ...(Array.isArray(manualFailureTargets) ? manualFailureTargets : [])]).length,
            removed: 0,
            notFound: 0,
            unsafe: 0,
            failed: 1,
    ...
    if (idx3 >= 0) {
      ...
      if (superseding) {
        ...
        list3[idx3] = {
          ...
          status: 'Failed',
          error: msg,
          releaseState: current?.manualUpload === true ? 'failed_upload' : current?.releaseState,
          downloadPath: current?.manualUpload === true ? '' : current?.downloadPath,
          sourceCleanupPath: current?.manualUpload === true ? '' : current?.sourceCleanupPath,
          holdDir: current?.manualUpload === true ? null : current?.holdDir,
          finalDir: current?.manualUpload === true ? null : current?.finalDir,
          finalTargetDir: current?.manualUpload === true ? null : current?.finalTargetDir,
          finalVideo: current?.manualUpload === true ? null : current?.finalVideo,
          manualUploadCleanup: current?.manualUpload === true ? manualCleanup : current?.manualUploadCleanup || null,
        };
      }
      db3[key] = list3;
    ...
    await appendProcessingLog({
      ...
      status: 'error',
      error: msg,
      summary: manualCleanup ? { manualUploadCleanup: manualCleanup } : undefined,
    });
    throw new Error(msg);
  } finally {
    await ssh.close();
...
export async function processNextCompleted({ type = 'all', limit = 1, all = false } = {}) {
  ...
  try {
    ...
    if (engineHost && mount?.mountDir) {
      ...
      try {
        for (const t of types) {
          ...
          const recoveryList = Array.isArray(db[key]) ? db[key] : list;
          const recoveryRes = await recoverStaleProcessingRowsForType({
            ssh,
            mountDir: mount.mountDir,
            settings,
            type: t,
          ...
          processingRecovery.types.push({
            type: t,
            scanned: Number(recoveryRes?.scanned || 0),
            recovered: Number(recoveryRes?.recovered || 0),
            skipped: Number(recoveryRes?.skipped || 0),
            changed: Number(recoveryRes?.changed || 0),
          });
          processingRecovery.scanned += Number(recoveryRes?.scanned || 0);
          processingRecovery.recovered += Number(recoveryRes?.recovered || 0);
          processingRecovery.skipped += Number(recoveryRes?.skipped || 0);
          processingRecovery.changed += Number(recoveryRes?.changed || 0);
          if (Number(recoveryRes?.changed || 0) > 0) {
            db[key] = Array.isArray(recoveryRes?.list) ? recoveryRes.list : recoveryList;
            db.processingLogs = Array.isArray(db.processingLogs) ? db.processingLogs : [];
            db.processingLogs.unshift(...(Array.isArray(recoveryRes?.logs) ? recoveryRes.logs : []));
        ...
        if (dbChanged) {
          await saveAdminDb(db);
          if (Number(processingRecovery.recovered || 0) > 0) clearPublicCatalogDataCache('public-upcoming:');
        }
      } finally {
        await ssh.close();
      }
      ...
      processingRecovery.reason = 'missing_engine_or_mount';
    }
  } catch (e) {
    ...
    processingRecovery.ok = false;
    processingRecovery.error = e?.message || 'Processing recovery failed.';
  }
  ...
  if (!queue.length) return { ok: true, skipped: true, stageCleanup, staleSourceCleanup, processingRecovery };
  if (queue.length === 1) {
    const one = await processOneCompleted({ type: queue[0].type, id: queue[0].id });
    return { ...one, stageCleanup, staleSourceCleanup, processingRecovery };
  }
