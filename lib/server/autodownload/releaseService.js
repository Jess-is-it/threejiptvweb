  ...
  try {
    ...
    const releaseOrphansForQueue = async (queueKey, typeNorm) => {
      ...
      for (let i = 0; i < rows.length; i += 1) {
        ...
        // leaving holdDir pointing at a folder that no longer exists. Clear these pointers so the
        // processing folder doesn't look artificially full in the UI/diagnostics.
        if (statusLower === 'deleted' && holdDir && holdDir.startsWith(processingDir)) {
          const exists = ssh ? await dirExists({ ssh, dir: holdDir }).catch(() => false) : false;
          if (!exists) {
            rows[i] = {
              ...rec,
