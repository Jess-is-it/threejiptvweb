...
import { getAutodownloadSettings, getEngineHost, updateXuiScanState } from './autodownloadDb';
    ...
    (Array.isArray(items) ? items : [])
      ...
      .map((row) => {
        ...
        return {
          ...
          image: poster ? `https://image.tmdb.org/t/p/w500${poster}` : '/images/placeholder.png',
          backdropImage: backdrop ? `https://image.tmdb.org/t/p/w1280${backdrop}` : '',
          href:
            mediaType === 'tv'
...
export async function releaseDueSelections({ type = 'all' } = {}) {
  ...
  if (!dueLogs.length) return { ok: true, skipped: true, reason: 'no_due_release' };
  ...
  try {
    ...
    for (const log of dueLogs) {
      ...
      for (let i = 0; i < rows.length; i += 1) {
        ...
        if (status === 'cleaned' && rec?.holdDir && rec?.finalTargetDir) {
          if (!ssh) {
            failed += 1;
            rows[i] = {
              ...rec,
  ...
  if (itemsNeedingTorrentDelete.length) {
    for (const row of itemsNeedingTorrentDelete) {
      try {
        await controlDownload({ type: row.type, id: row.id, action: 'delete' });
      } catch {}
    }
  }
