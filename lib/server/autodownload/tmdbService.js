...
const TMDB_RETRY_STATUSES = new Set([429, 500, 502, 503, 504]);
...
async function tmdb(path, apiKey, params) {
  const url = withKey(`${TMDB_BASE}${path}`, apiKey, params);
  let lastStatus = 0;
  for (let attempt = 1; attempt <= TMDB_MAX_ATTEMPTS; attempt += 1) {
    const r = await fetch(url, { cache: 'no-store' });
    if (r.ok) return r.json();
    lastStatus = Number(r.status || 0) || 0;
    if (!TMDB_RETRY_STATUSES.has(lastStatus) || attempt >= TMDB_MAX_ATTEMPTS) {
      throw new Error(`TMDB ${lastStatus || 'request failed'}`);
    }
