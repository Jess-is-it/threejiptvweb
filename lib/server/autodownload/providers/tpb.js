...
function parseSizeFromRow(rowText) {
  const decoded = decodeHtmlEntities(String(rowText || '')).replace(/&nbsp;/gi, ' ');
  const directCell = decoded.match(/<td[^>]*align=["']?right["']?[^>]*>\s*([0-9]+(?:\.[0-9]+)?\s*(?:[kmgt]i?b|bytes?))\s*<\/td>/i);
  if (directCell?.[1]) return parseSizeToGb(directCell[1]);
  const legacy = decoded.match(/size\s*([0-9]+(?:\.[0-9]+)?)\s*([kmgt]i?b|bytes?)/i);
  if (!legacy) return null;
  return parseSizeToGb(`${legacy[1]} ${legacy[2]}`);
}
...
function parseTitleFromRow(rowHtml) {
  ...
  for (const match of anchors) {
    ...
    if (!/\/torrent\//i.test(href) && !/^details for /i.test(titleAttr)) continue;
    const innerText = decodeHtmlEntities(inner).replace(/<[^>]+>/g, '').trim();
    const attrText = decodeHtmlEntities(titleAttr).replace(/^details for\s+/i, '').trim();
    const value = innerText || attrText;
...
function parseResults(html, domainUsed) {
  ...
  for (const row of rows) {
    ...
    const name = parseTitleFromRow(rowHtml) || 'Unknown title';
