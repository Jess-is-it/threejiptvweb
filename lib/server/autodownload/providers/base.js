...
async function readResponseTextLimited(response, { maxBytes = DEFAULT_MAX_RESPONSE_BYTES } = {}) {
  const limit = Math.max(256 * 1024, Math.floor(Number(maxBytes || DEFAULT_MAX_RESPONSE_BYTES)));
  const contentLength = Number(response.headers.get('content-length') || 0) || 0;
  if (contentLength > limit) {
    throw new ProviderError(`Provider response too large (${formatBytes(contentLength)}; limit ${formatBytes(limit)}).`, {
  ...
  if (!response.body || typeof response.body.getReader !== 'function') {
    ...
    if (Buffer.byteLength(text, 'utf8') > limit) {
      throw new ProviderError(`Provider response too large (limit ${formatBytes(limit)}).`, {
        category: ERROR_CATEGORY.PARSE,
        httpStatus: response.status,
        url: response.url || '',
      });
  ...
  try {
    for (;;) {
      ...
      if (total > limit) {
        ...
        throw new ProviderError(`Provider response too large (over ${formatBytes(limit)}).`, {
          category: ERROR_CATEGORY.PARSE,
          httpStatus: response.status,
          url: response.url || '',
        });
...
export async function fetchWithTimeout(url, opts = {}) {
  ...
  const maxResponseBytes = Math.max(256 * 1024, Number(opts.maxResponseBytes || DEFAULT_MAX_RESPONSE_BYTES) || DEFAULT_MAX_RESPONSE_BYTES);
  const headers = {
  ...
  try {
    ...
    const text = await readResponseTextLimited(res, { maxBytes: maxResponseBytes });
