...
function stripQualitySuffix(value) {
  return String(value || '')
...
function parseTitleYear(folderName) {
  ...
  const cleaned = stripQualitySuffix(base);
  const m = cleaned.match(/^(.*?)(?:\s*\((\d{4})\)|\s+(\d{4}))?$/);
  const rawTitle = normalizeCountSuffix(m?.[1] || cleaned);
  const yearNum = Number(m?.[2] || m?.[3] || 0);
  const year = Number.isFinite(yearNum) && yearNum >= 1900 && yearNum <= 2100 ? yearNum : null;
  return { title: rawTitle || cleaned || base, year };
}
...
function normalizeInventory(raw) {
  ...
  next.movies = uniqBy(
    (Array.isArray(next.movies) ? next.movies : []).map((row) => {
      const fileSource = String(row?.fileName || '').split('/').filter(Boolean).pop() || '';
      const pathSource = String(row?.path || '').split('/').filter(Boolean).pop() || '';
      const { title, year } = parseTitleYear(fileSource || pathSource || row?.title || '');
      return {
        ...row,
        title,
        year,
        key: makeLibraryInventoryKey(title, year),
        normalizedTitle: normalizeTitle(title),
      };
    }),
    (row) => String(row?.key || '').trim() || String(row?.path || '').trim()
  );
  next.series = uniqBy(
    (Array.isArray(next.series) ? next.series : []).map((row) => {
      const folderSource = String(row?.folder || '').split('/').filter(Boolean).pop() || '';
      const pathSource = String(row?.path || '').split('/').filter(Boolean).pop() || '';
      const { title, year } = parseTitleYear(folderSource || pathSource || row?.title || '');
      return {
        ...row,
        title,
        year,
  ...
  const movieCount = next.movies.length;
  const seriesCount = next.series.length;
  next.stats = next.stats && typeof next.stats === 'object' ? { ...base.stats, ...next.stats } : base.stats;
  next.stats.movies = movieCount;
  next.stats.series = seriesCount;
  next.stats.total = movieCount + seriesCount;
  return next;
}
...
function buildVideoFindExpr() {
  // Keep this conservative: only common container formats.
  // This prevents huge inventories from subtitles/posters/nfo files.
  const exts = ['mkv', 'mp4', 'avi', 'mov', 'wmv', 'm4v', 'ts', 'm2ts'];
  return exts.map((x) => `-iname "*.${x}"`).join(' -o ');
}

async function listMovieInventoryCandidates(ssh, moviesRoot) {
  // Movies library layout varies (some are folders, some are flat files). Scanning *all* files is expensive
  // and can exhaust memory. Instead:
  // 1) Treat depth-3 folders as the canonical movie "title folders" (Movies/{Category}/{Genre}/{Title...})
  // 2) Also include depth-3 video files for flat layouts (Movies/{Category}/{Genre}/{Title...}.mkv)
  const videoExpr = buildVideoFindExpr();
  const cmd = [
    'set -e',
    `ROOT=${shQuote(moviesRoot)}`,
    'if [ ! -d "$ROOT" ]; then exit 0; fi',
    'find "$ROOT" -mindepth 3 -maxdepth 3 -type d -print 2>/dev/null || true',
    `find "$ROOT" -mindepth 3 -maxdepth 3 -type f \\( ${videoExpr} \\) -print 2>/dev/null || true`,
  ].join('\n');
  const r = await ssh.exec(cmd, { timeoutMs: 120000 });
  if (Number(r?.code || 0) !== 0) {
    const msg = String(r?.stderr || r?.stdout || '').trim();
    throw new Error(msg || `Failed to scan ${moviesRoot}`);
  }
  return String(r?.stdout || '')
    .split(/\r?\n/)
    .map((x) => x.trim())
...
export async function syncLibraryInventory() {
  ...
  try {
    ...
    const [movieCandidates, seriesDirs] = await Promise.all([
      listMovieInventoryCandidates(ssh, moviesRoot),
      listPaths(ssh, seriesRoot, 2, 2, 'd'),
    ]);

    const movies = buildMovieInventory(movieCandidates, moviesRoot);
