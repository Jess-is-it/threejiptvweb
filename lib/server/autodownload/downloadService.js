  ...
  try {
    ...
    for (const t of types) {
      ...
      for (const rec of list) {
        ...
        const tor = byHash.get(normalizeHash(rec.qbHash));
        if (!tor) {
        ...
        const statusLower = String(next.status || '').toLowerCase();
        if (autoDeleteCompletedTorrents && isAutoDeleteEligibleStatus(statusLower) && next?.qbHash && !next?.qbDeletedAt) {
          ...
          if (now() >= dueAt) {
            try {
              await qbCurl(ssh, {
                ...
                path: '/api/v2/torrents/delete',
                method: 'POST',
                form: { hashes: String(next.qbHash), deleteFiles: 'false' },
                cookieJar: qbSession.cookieJar,
                timeoutMs: 60000,
              });
              next = {
