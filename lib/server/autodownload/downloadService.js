...
import { getSeriesPipeline } from './seriesPipelines';
...
const VPN_NOT_READY_PAUSE_REASON = 'Paused: VPN is required but not ready.';
...
function isReleasedRecord(record = {}) {
  const releaseState = String(record?.releaseState || '').trim().toLowerCase();
  const status = String(record?.status || '').trim().toLowerCase();
  return releaseState === 'released' || status === 'released';
}

function normalizeReleasedRecord(record = {}) {
  const next = {
    ...record,
    status: 'Released',
  ...
  if (next.qbHash && next.qbDeletedAt) next.qbHash = null;
  delete next.timedOutAt;
...
function sourcePolicyForRecord(settings, type, rec = {}) {
  const base = sourcePolicyForType(settings, type);
  if (sanitizeType(type) !== 'series') return base;

  const pipelineKey = String(rec?.seriesMeta?.pipelineKey || '').trim();
  if (!pipelineKey) return base;

  const pipeline = getSeriesPipeline(settings, pipelineKey);
  const mode = String(rec?.seriesMeta?.mode || '').trim().toLowerCase();
  return {
    ...base,
    minSeeders: Math.max(0, Math.floor(toFiniteNumber(pipeline?.minSeeders, base.minSeeders) ?? base.minSeeders)),
    maxSizeGb:
      mode === 'season_pack'
        ? toFiniteNumber(pipeline?.maxSeasonTotalGb, base.maxSeasonTotalGb)
        : toFiniteNumber(pipeline?.maxEpisodeGb, base.maxSizeGb),
    maxEpisodeGb: toFiniteNumber(pipeline?.maxEpisodeGb, base.maxEpisodeGb),
    maxSeasonTotalGb: toFiniteNumber(pipeline?.maxSeasonTotalGb, base.maxSeasonTotalGb),
  };
}
...
function pad2(n) {
  return String(Number(n || 0) || 0).padStart(2, '0');
}
...
function scoreSourceCandidate(source = {}) {
  const seed = Math.max(0, Number(source?.seeders || 0) || 0);
  const size = Number(source?.sizeGb || 0) || 0;
  return seed * 10 + qualityScore(source?.quality) - Math.min(50, size);
}

function isSeasonPackSourceName(name, seasonNumber = 1) {
  const s = String(name || '').toLowerCase().replace(/[._]+/g, ' ');
  const sn = Number(seasonNumber || 1) || 1;
  const hasSeason =
    new RegExp(`\\bs0?${sn}\\b`).test(s) ||
    new RegExp(`\\bseason\\s*0?${sn}\\b`).test(s) ||
    new RegExp(`\\bcomplete\\s+season\\s*0?${sn}\\b`).test(s);
  ...
  if (sn === 1 && new RegExp(`\\bs0?${Number(pad2(sn)) + 1}\\b`).test(s)) return false;
  return true;
}

function seasonPackSourceMatchesPolicy(source, policy, seasonNumber = 1) {
  if (!sourceCandidateMatchesPolicy(source, policy)) return false;
  if (!isSeasonPackSourceName(source?.name || source?.title || '', seasonNumber)) return false;
  return Boolean(String(source?.sourceUrl || source?.magnet || source?.url || source?.link || source?.hash || '').trim());
}
...
function pickBestSeasonPackSource(rows, { policy, seasonNumber = 1 } = {}) {
  const valid = (Array.isArray(rows) ? rows : []).filter((row) => seasonPackSourceMatchesPolicy(row, policy, seasonNumber));
  return [...valid].sort((a, b) => scoreSourceCandidate(b) - scoreSourceCandidate(a))[0] || null;
}
...
async function searchBestSeasonPackDownloadSource({
  rec,
  query,
  year = null,
  selectionLogId = '',
  ...
  const seasonNumber = Math.max(1, Number(rec?.seriesMeta?.season || 1) || 1);
  const title = String(rec?.tmdb?.title || rec?.title || '').trim();
  const queries = uniqueStrings([
    query,
    title ? `${title} S${pad2(seasonNumber)}` : '',
    title ? `${title} Season ${seasonNumber}` : '',
  ]);
  let out = { ok: false, selected: null, candidates: [], attempts: [], error: '' };
  for (const sourceQuery of queries) {
    const result = await searchBestDownloadSource({
      query: sourceQuery,
      year,
      type: 'series',
      selectionLogId,
      ...
      minSeeders: policy.minSeeders,
      maxSizeGb: policy.maxSizeGb,
      maxResults: 75,
    });
    const candidates = uniqueSourceCandidates([...(out.candidates || []), ...(Array.isArray(result?.candidates) ? result.candidates : [])]);
    const selected = pickBestSeasonPackSource(candidates, { policy, seasonNumber });
    out = {
      ok: Boolean(selected),
      selected,
      candidates,
      attempts: [...(out.attempts || []), ...(Array.isArray(result?.attempts) ? result.attempts : [])],
      error: result?.error || out.error || '',
    };
    if (selected) break;
  }
  if (!out.selected) {
    out.error = `No valid Season ${seasonNumber} TV pack source found${
      policy?.maxSizeGb ? ` within ${policy.maxSizeGb} GB` : ''
    }.`;
  }
  return out;
}
...
async function reconcileSelectionLogReplacement({ logId, removedTmdbId, replacementItem = null } = {}) {
  ...
  if (replacementItem && replacementTmdbId > 0) {
    nextLog.errorMessage = null;
    if (nextLog?.replacementFailure && typeof nextLog.replacementFailure === 'object') {
      const replacementFailure = { ...nextLog.replacementFailure };
      const selectionType = String(nextLog?.selectionType || '').trim().toLowerCase();
      if (selectionType && Object.prototype.hasOwnProperty.call(replacementFailure, selectionType)) {
        delete replacementFailure[selectionType];
      }
      nextLog.replacementFailure = Object.keys(replacementFailure).length ? replacementFailure : null;
    }
  }
  if (removedRow) {
...
function shouldAutoStartQueuedRecord(rec, at = now()) {
  ...
  if (isReleasedRecord(rec)) return false;
...
export async function pauseManagedDownloadsForVpnNotReady({ type = 'all', summary = '' } = {}) {
  const scope = type === 'movie' || type === 'series' ? type : 'all';
  const note = String(summary || '').trim();
  const message = note ? `${VPN_NOT_READY_PAUSE_REASON} (${note})` : VPN_NOT_READY_PAUSE_REASON;
  const at = now();
  const out = { ok: true, changed: 0, scope };
  ...
  const applyTo = (key) => {
    ...
    const next = list.map((row) => {
      ...
      if (isTerminalDownloadStatus(statusLower)) return row;
      if (String(row?.systemPauseReason || '').trim().toLowerCase() === 'mount_offline') return row;
      const managed = Boolean(String(row?.selectionLogId || '').trim());
      const queued = statusLower === 'queued';
      if (!managed || !queued || row?.qbHash) return row;
      const curErr = String(row?.error || '').trim();
      if (curErr === message && String(row?.systemPauseReason || '').trim().toLowerCase() === 'vpn_not_ready') return row;
      changed += 1;
      return {
        ...row,
        status: 'Queued',
        error: message,
        systemPauseReason: 'vpn_not_ready',
        systemPausedAt: row?.systemPausedAt || at,
      };
    });
    if (changed) {
      db[key] = next;
  ...
  if (scope === 'all' || scope === 'movie') applyTo('downloadsMovies');
  if (scope === 'all' || scope === 'series') applyTo('downloadsSeries');

  if (out.changed) await saveAdminDb(db);
  return out;
}

export async function resumeManagedDownloadsPausedForVpn({ type = 'all' } = {}) {
  const scope = type === 'movie' || type === 'series' ? type : 'all';
  const at = now();
  const out = { ok: true, changed: 0, scope };
  ...
  const applyTo = (key) => {
    ...
    let changed = 0;
    const next = list.map((row) => {
      const reason = String(row?.systemPauseReason || '').trim().toLowerCase();
      if (reason !== 'vpn_not_ready') return row;
      const statusLower = String(row?.status || '').trim().toLowerCase();
      if (statusLower !== 'queued' || row?.qbHash) return row;
      changed += 1;
      const curErr = String(row?.error || '').trim();
      return {
        ...row,
        error: curErr.startsWith(VPN_NOT_READY_PAUSE_REASON) ? '' : curErr,
        systemPauseReason: '',
        systemResumedAt: at,
      };
    });
  ...
  try {
    ...
    for (const t of types) {
      ...
      for (const rec of candidates) {
        ...
        const sourcePolicy = sourcePolicyForRecord(settings, t, rec);
        const seriesMode = String(rec?.seriesMeta?.mode || '').trim().toLowerCase();
        const requiresSeriesPack = t === 'series' && seriesMode === 'season_pack';
        const effectiveMaxSizeGb =
          requiresSeriesPack ? sourcePolicy.maxSeasonTotalGb : sourcePolicy.maxSizeGb;
        const effectivePolicy = { ...sourcePolicy, maxSizeGb: effectiveMaxSizeGb };

        const allowExistingSeriesContinuation =
          t === 'series' && String(rec?.seriesMeta?.pipelineKey || '').trim() === 'existingSeries';
        ...
        if (duplicateInLibrary && !allowExistingSeriesContinuation) {
          await updateRecordById({
            type: t,
            id: rec.id,
            mutate: (cur) => ({
        ...
        if (duplicateInXui && !allowExistingSeriesContinuation) {
          await updateRecordById({
            type: t,
            id: rec.id,
            mutate: (cur) => ({
        ...
        let selected =
          (requiresSeriesPack
            ? seasonPackSourceMatchesPolicy(rec?.source, effectivePolicy, rec?.seriesMeta?.season || 1)
        ...
        if (!sourceLink) {
          ...
          try {
            sourceResult = requiresSeriesPack
              ? await searchBestSeasonPackDownloadSource({
                  rec,
                  query,
                  year,
                  selectionLogId,
              ...
              : await searchBestDownloadSource({
                  query,
                  year,
                  type: t,
                  selectionLogId,
          ...
          if (
            selected &&
            (requiresSeriesPack
              ? !seasonPackSourceMatchesPolicy(selected, effectivePolicy, rec?.seriesMeta?.season || 1)
              : !sourceCandidateMatchesPolicy(selected, effectivePolicy))
          ) {
            sourceResult = {
              ...(sourceResult || {}),
              ok: false,
              error: requiresSeriesPack
                ? `Filtered by TV-pack policy (Season ${rec?.seriesMeta?.season || 1}, min seeders ${sourcePolicy.minSeeders}${
                    effectivePolicy.maxSizeGb !== null ? `, max pack size ${effectivePolicy.maxSizeGb} GB` : ''
                  }).`
                : `Filtered by source policy (min seeders ${sourcePolicy.minSeeders}${
  ...
  try {
    ...
    for (const t of types) {
      ...
      for (const rec of list) {
        const recReleased = isReleasedRecord(rec);
        const selectionLogId = String(rec?.selectionLogId || '').trim();
        ...
        if (!rec?.qbHash) {
          if (recReleased) {
            nextList.push(normalizeReleasedRecord(rec));
            continue;
          }
        ...
        if (!tor) {
          ...
          const isFinal = ['completed', 'processing', 'cleaned', 'released', 'deleted'].includes(statusLower) || releaseStateLower === 'released';
          if (!isFinal) {
            ...
            const missingFinal = recReleased ? normalizeReleasedRecord(rec) : { ...rec };
        ...
        if (recReleased) {
          let releasedNext = normalizeReleasedRecord({
            ...rec,
            qbName: tor?.name || rec?.qbName || null,
            progress: Number(tor?.progress || 0),
            sizeBytes: Number(tor?.size || 0) || rec?.sizeBytes,
          });
          if (autoDeleteCompletedTorrents && releasedNext?.qbHash && !releasedNext?.qbDeletedAt) {
            const completedAtMs = resolveCompletedAtMs({ record: releasedNext, torrent: tor });
            ...
            if (now() >= dueAt) {
              try {
                await qbDeleteAndConfirm({
                  ...
                  port,
                  cookieJar: qbSession.cookieJar,
                  hash: String(releasedNext.qbHash),
                  deleteFiles: false,
                });
                releasedNext = {
                  ...releasedNext,
        ...
        const terminalStatus = isTerminalDownloadStatus(next.status);
        const placement = await enforceTorrentPlacement({
          ssh,
          port,
          cookieJar: qbSession.cookieJar,
          qbHash: next.qbHash,
          desiredLocation: terminalStatus ? desiredDownloaded : desiredDownloading,
        ...
        if (placement.moved) {
          next = {
            ...next,
            savePath: terminalStatus ? desiredDownloaded : desiredDownloading,
          };
        }
...
export async function controlDownload({ type, id, action, deleteArtifacts = true, preserveSelectionLog = false } = {}) {
  ...
  try {
    if (a === 'pause') {
      ...
      if (isReleasedRecord(rec)) throw new Error('Retry is locked because this item is already deployed in XUI.');
      qbSession = await createQbSession({ ssh, port, settings });
  ...
  try {
    ...
    for (const key of types) {
      ...
      for (let i = 0; i < list.length; i++) {
        const row = list[i];
        if (isReleasedRecord(row)) continue;
      ...
      for (let i = 0; i < list.length; i++) {
        ...
        if (isReleasedRecord(rec)) {
          list[i] = normalizeReleasedRecord(rec);
          continue;
        }
        if (!recHash) continue;
        ...
        const pipelineKey = String(rec?.seriesMeta?.pipelineKey || '').trim();
        const pipelineTimeoutHours = key === 'downloadsSeries' && pipelineKey ? getSeriesPipeline(settings, pipelineKey).timeoutHours : 0;
        const recordTimeoutHoursRaw =
          key === 'downloadsSeries' ? Number(rec?.seriesMeta?.timeoutHours || pipelineTimeoutHours || 0) : 0;
        const recordTimeoutHours =
          Number.isFinite(recordTimeoutHoursRaw) && recordTimeoutHoursRaw > 0 ? recordTimeoutHoursRaw : maxWaitHours;
        const cutoffMs = recordTimeoutHours * 60 * 60 * 1000;
