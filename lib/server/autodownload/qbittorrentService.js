...
function resolveQbServiceAccount(downloadClient = {}) {
  const dc = downloadClient && typeof downloadClient === 'object' ? downloadClient : {};
  const serviceUser = sanitizeLinuxAccountName(dc.serviceUser, 'qbvpn');
  const serviceGroup = sanitizeLinuxAccountName(dc.serviceGroup, serviceUser);
  return { serviceUser, serviceGroup };
}

function qbProfileHome(serviceUser) {
  return `/home/${sanitizeLinuxAccountName(serviceUser, 'qbvpn')}`;
}
...
async function fetchQbRuntimeQueueOptionsFromConfig({ ssh, serviceUser = 'qbvpn' } = {}) {
  const svcUser = sanitizeLinuxAccountName(serviceUser, 'qbvpn');
  const svcHome = qbProfileHome(svcUser);
  const cmd = `
CONF_A=${JSON.stringify(`${svcHome}/qBittorrent/config/qBittorrent.conf`)}
CONF_B=${JSON.stringify(`${svcHome}/.config/qBittorrent/qBittorrent.conf`)}
CONF_C="/home/xui/qBittorrent/config/qBittorrent.conf"
CONF_D="/home/xui/.config/qBittorrent/qBittorrent.conf"
if [ -f "$CONF_A" ]; then
  cat "$CONF_A"
elif [ -f "$CONF_B" ]; then
  cat "$CONF_B"
...
export async function getQbittorrentSettingsSafe({ syncRuntime = false } = {}) {
  ...
  const { serviceUser, serviceGroup } = resolveQbServiceAccount(dc);
...
export async function revealQbittorrentWebUiPassword() {
  const db = await getAdminDb();
  const dc = db.autodownloadSettings?.downloadClient || {};
  const password = dc?.passwordEnc ? decryptString(dc.passwordEnc) : String(dc?.password || '');
  const value = String(password || '');
  if (!value) throw new Error('No qBittorrent WebUI password is saved yet.');
  return value;
}
...
export async function updateQbittorrentSettingsFromAdminInput(input) {
  ...
  const { serviceUser, serviceGroup } = resolveQbServiceAccount(prev);
...
export async function provisionQbittorrentNox({ port = 8080, username = '', password = '', connectionMode = null, lanBind = null } = {}) {
  ...
  const dc = db.autodownloadSettings?.downloadClient || {};
  const { serviceUser, serviceGroup } = resolveQbServiceAccount(dc);
  const serviceHome = qbProfileHome(serviceUser);
...
QBIT_SYS_USER=${JSON.stringify(serviceUser)}
QBIT_SYS_GROUP=${JSON.stringify(serviceGroup)}
QBIT_HOME=${JSON.stringify(serviceHome)}
...
ensure_user() {
  if ! getent group "$QBIT_SYS_GROUP" >/dev/null 2>&1; then
    groupadd --system "$QBIT_SYS_GROUP"
  fi
  if ! id -u "$QBIT_SYS_USER" >/dev/null 2>&1; then
    useradd --system --home "$QBIT_HOME" --create-home --shell /usr/sbin/nologin -g "$QBIT_SYS_GROUP" "$QBIT_SYS_USER"
  else
    usermod -d "$QBIT_HOME" "$QBIT_SYS_USER" >/dev/null 2>&1 || true
    usermod -g "$QBIT_SYS_GROUP" "$QBIT_SYS_USER" >/dev/null 2>&1 || true
  fi
  install -d -m 0755 -o "$QBIT_SYS_USER" -g "$QBIT_SYS_GROUP" "$QBIT_HOME"
  chown "$QBIT_SYS_USER:$QBIT_SYS_GROUP" "$QBIT_HOME" >/dev/null 2>&1 || true
  chmod 0755 "$QBIT_HOME" >/dev/null 2>&1 || true
}

ensure_profile_dirs() {
  install -d -m 0755 -o "$QBIT_SYS_USER" -g "$QBIT_SYS_GROUP" "$QBIT_HOME/qBittorrent"
  install -d -m 0755 -o "$QBIT_SYS_USER" -g "$QBIT_SYS_GROUP" "$QBIT_HOME/qBittorrent/config"
  install -d -m 0755 -o "$QBIT_SYS_USER" -g "$QBIT_SYS_GROUP" "$QBIT_HOME/qBittorrent/cache"
  chown -R "$QBIT_SYS_USER:$QBIT_SYS_GROUP" "$QBIT_HOME/qBittorrent" >/dev/null 2>&1 || true
  if [ -d "$QBIT_HOME/.config/qBittorrent" ]; then
    chown -R "$QBIT_SYS_USER:$QBIT_SYS_GROUP" "$QBIT_HOME/.config/qBittorrent" >/dev/null 2>&1 || true
  fi
}

ensure_save_permissions() {
  for P in "$MOVIES_SAVE" "$SERIES_SAVE"; do
    if [ -z "$P" ]; then
      continue
    fi
    if [ ! -d "$P" ]; then
      install -d -m 2775 -o "$QBIT_SYS_USER" -g "$QBIT_SYS_GROUP" "$P" >/dev/null 2>&1 || mkdir -p "$P" >/dev/null 2>&1 || true
    fi
    if [ ! -d "$P" ]; then
      continue
    fi
    gid_num="$(stat -c %g "$P" 2>/dev/null || true)"
    grp_name=""
    if [ -n "$gid_num" ]; then
      grp_name="$(getent group "$gid_num" 2>/dev/null | cut -d: -f1 | head -n1 || true)"
      if [ -z "$grp_name" ] && [ "$gid_num" != "0" ]; then
        grp_name="qbmount_$gid_num"
        getent group "$grp_name" >/dev/null 2>&1 || groupadd -g "$gid_num" "$grp_name" >/dev/null 2>&1 || true
      fi
    fi
    if [ -z "$grp_name" ] || [ "$grp_name" = "root" ]; then
      grp_name="$QBIT_SYS_GROUP"
      chgrp "$grp_name" "$P" >/dev/null 2>&1 || true
    fi
    usermod -a -G "$grp_name" "$QBIT_SYS_USER" >/dev/null 2>&1 || true
    chmod g+rwx "$P" >/dev/null 2>&1 || true
    chmod g+s "$P" >/dev/null 2>&1 || true
  done
}
...
# qBittorrent uses this config path when running with --profile=$QBIT_HOME.
CONF_DIR="$QBIT_HOME/qBittorrent/config"
# Legacy path from earlier builds (kept for migration).
LEGACY_CONF_DIR="$QBIT_HOME/.config/qBittorrent"
LEGACY_XUI_CONF_DIR="/home/xui/.config/qBittorrent"
LEGACY_XUI_PROFILE_DIR="/home/xui/qBittorrent/config"
CONF_FILE="$CONF_DIR/qBittorrent.conf"
install -d -m 0755 -o "$QBIT_SYS_USER" -g "$QBIT_SYS_GROUP" "$CONF_DIR"
...
if [ ! -f "$CONF_FILE" ] && [ -f "$LEGACY_XUI_CONF_DIR/qBittorrent.conf" ]; then
  cp "$LEGACY_XUI_CONF_DIR/qBittorrent.conf" "$CONF_FILE" >/dev/null 2>&1 || true
fi
if [ ! -f "$CONF_FILE" ] && [ -f "$LEGACY_XUI_PROFILE_DIR/qBittorrent.conf" ]; then
  cp "$LEGACY_XUI_PROFILE_DIR/qBittorrent.conf" "$CONF_FILE" >/dev/null 2>&1 || true
fi
...
EOF
  chown "$QBIT_SYS_USER:$QBIT_SYS_GROUP" "$CONF_FILE"
fi
...
ExecStart=/usr/bin/qbittorrent-nox --webui-port=${webuiPort} --profile=${serviceHome}
Restart=always
RestartSec=3
