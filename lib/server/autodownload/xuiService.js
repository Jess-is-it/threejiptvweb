...
const DEFAULT_XUI_RESPONSE_LIMIT_BYTES = 40 * 1024 * 1024;
...
function encryptedSecretHint(payload) {
  if (!payload) return '';
  try {
    return maskSecret(decryptString(payload));
  } catch {
    return 'saved';
  }
}
...
async function readResponseTextLimited(response, { maxBytes = DEFAULT_XUI_RESPONSE_LIMIT_BYTES } = {}) {
  const limit = Math.max(1024 * 1024, Math.floor(Number(maxBytes || DEFAULT_XUI_RESPONSE_LIMIT_BYTES)));
  const contentLength = Number(response.headers.get('content-length') || 0) || 0;
  if (contentLength > limit) {
    throw new Error(`XUI response too large (${formatBytes(contentLength)}; limit ${formatBytes(limit)}).`);
  }

  if (!response.body || typeof response.body.getReader !== 'function') {
    const text = await response.text().catch(() => '');
    if (Buffer.byteLength(text, 'utf8') > limit) {
      throw new Error(`XUI response too large (limit ${formatBytes(limit)}).`);
    }
    return text;
  }
  ...
  try {
    for (;;) {
      ...
      if (total > limit) {
        ...
        throw new Error(`XUI response too large (over ${formatBytes(limit)}).`);
      }
      chunks.push(chunk);
    }
  } finally {
...
export async function getXuiIntegrationSafe() {
  ...
  return {
    ...
    hasPublicTunnelToken: Boolean(cfg.publicTunnelTokenEnc),
    publicTunnelTokenHint: encryptedSecretHint(cfg.publicTunnelTokenEnc),
...
export async function updateXuiIntegrationFromAdminInput(input) {
  ...
  const publicBaseUrl = normalizeOptionalBaseUrl(input?.publicBaseUrl ?? current?.publicBaseUrl ?? '');
  const accessCode = String(input?.accessCode || '').trim();
  const apiKey = String(input?.apiKey || '').trim();
  const publicTunnelTokenInput = String(input?.publicTunnelToken || input?.publicTunnelConnectorCommand || '').trim();
  ...
  const publicTunnelTokenEnc =
    input?.clearPublicTunnelToken === true
      ? null
      : publicTunnelTokenInput
        ? encryptString(extractCloudflareTunnelToken(publicTunnelTokenInput))
        : current?.publicTunnelTokenEnc;
  ...
  const next = {
    ...
    publicTunnelTokenEnc,
    publicTunnelTokenUpdatedAt: publicTunnelTokenInput
      ? new Date().toISOString()
      : input?.clearPublicTunnelToken === true
        ? ''
        : current?.publicTunnelTokenUpdatedAt || '',
    watchFolderIdMovies,
    watchFolderIdSeries,
  };
...
export async function xuiApiCall({ action, params = {}, maxResponseBytes = DEFAULT_XUI_RESPONSE_LIMIT_BYTES } = {}) {
  const cfg = await getXuiIntegration();
  ...
  const text = await readResponseTextLimited(r, { maxBytes: maxResponseBytes }).catch((error) => {
    throw new Error(error?.message || 'Failed to read XUI response.');
  });
  let data = null;
  try {
