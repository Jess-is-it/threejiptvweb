...
async function upsertSelectionLog(logEntry, { appendToLogId = null } = {}) {
  ...
  const merged = {
    ...prev,
    totalSelected: Math.max(0, Number(prev?.totalSelected || 0) + Number(logEntry?.totalSelected || 0)),
    recentAnimationSelected:
      Math.max(0, Number(prev?.recentAnimationSelected || 0) + Number(logEntry?.recentAnimationSelected || 0)),
    recentLiveActionSelected:
      Math.max(0, Number(prev?.recentLiveActionSelected || 0) + Number(logEntry?.recentLiveActionSelected || 0)),
    classicAnimationSelected:
      Math.max(0, Number(prev?.classicAnimationSelected || 0) + Number(logEntry?.classicAnimationSelected || 0)),
    classicLiveActionSelected:
      Math.max(0, Number(prev?.classicLiveActionSelected || 0) + Number(logEntry?.classicLiveActionSelected || 0)),
    ...
    selectedItems: [...(Array.isArray(prev?.selectedItems) ? prev.selectedItems : []), ...(logEntry?.selectedItems || [])],
    ...
    updatedAt: now(),
    errorMessage: logEntry?.errorMessage || prev?.errorMessage || null,
...
async function runSelectionJob({
  ...
  const limitPct = Number(settings?.storage?.limitPercent ?? 95) || 95;
  if (usedPct !== null && usedPct >= limitPct) {
    const health = (await getAutodownloadHealth()) || {};
    await setAutodownloadHealth({
      ...health,
      selection: { ok: false, type: t, lastRunAt: startedAt, error: `Storage limit reached (${Math.round(usedPct)}%)` },
    });
  ...
  const cappedTarget = Number.isFinite(Number(targetTotal)) ? Math.max(0, Math.floor(Number(targetTotal))) : null;
  if (cappedTarget !== null) {
    let remaining = cappedTarget;
    for (const b of buckets) {
      const wanted = Math.max(0, Number(b?.count || 0));
      const nextCount = Math.min(wanted, Math.max(0, remaining));
      b.count = nextCount;
      remaining -= nextCount;
    }
  }
  ...
  try {
    ...
    for (const b of buckets) {
      if (!b.count) continue;
      const candidates = await collectCandidates({ params: b.discover, maxPages: 8, discoverFn });
      let got = 0;
