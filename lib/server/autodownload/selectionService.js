...
async function runSelectionJob({
  ...
  const bootstrapMissingSeriesToS01E01 = t === 'series' ? settings?.selection?.seriesBootstrapMissingToS01E01 !== false : false;
  ...
  try {
    ...
    for (const b of buckets) {
      ...
      candidates = candidates.filter((c) => {
        ...
        if (t !== 'series' && title && inventoryIndexHas({ index: inventoryIndex, title, year })) {
          // For movies: skip items already present on NAS.
          // For series: we may want to keep selecting existing shows to fetch episodes.
          dedupeIds.add(id);
          return false;
        }
        return true;
      ...
      for (const c of candidates) {
        ...
        const existsInInventory = inventoryIndexHas({ index: inventoryIndex, title: details.title, year: details.year });
        const episodeHint =
          t === 'series' && bootstrapMissingSeriesToS01E01 && !existsInInventory ? { season: 1, episode: 1 } : null;
