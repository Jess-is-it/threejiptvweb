...
async function runSelectionJob({
  ...
  const bootstrapMissingSeriesToSeason1 =
    t === 'series'
      ? (settings?.selection?.seriesBootstrapMissingToSeason1 ?? settings?.selection?.seriesBootstrapMissingToS01E01) !== false
      : false;
  const maxSeasonTotalGbRaw = t === 'series' ? Number(settings?.sizeLimits?.maxSeasonTotalGb ?? 0) : 0;
  const maxSeasonTotalGb = Number.isFinite(maxSeasonTotalGbRaw) && maxSeasonTotalGbRaw > 0 ? maxSeasonTotalGbRaw : null;
  ...
  try {
    ...
    for (const b of buckets) {
      ...
      for (const c of candidates) {
        ...
        if (t === 'series' && bootstrapMissingSeriesToSeason1) {
          ...
          if (packOk) {
            ...
            for (const ep of episodeNums) {
              const eps = pool.filter((row) => isEpisodeName(row?.name || row?.title || '', 1, ep));
              const best = pickBest(eps);
              const gb = best?.sizeGb === null || best?.sizeGb === undefined ? NaN : Number(best.sizeGb);
              if (!best || !Number.isFinite(gb) || gb <= 0) {
                missing = true;
                break;
              }
              if (sourcePolicy.maxSizeGb !== null && gb > sourcePolicy.maxSizeGb) {
                missing = true;
                break;
              }
              pickedEpisodes.push({ ep, source: best, gb });
              totalGb += gb;
              if (maxSeasonTotalGb !== null && totalGb > maxSeasonTotalGb) {
                missing = true;
                break;
              }
            }

            if (missing || !pickedEpisodes.length) {
              skippedNoSourceCount++;
              continue;
            }

            for (const item of pickedEpisodes) {
              await addQueueItemFromTmdb({
                type: t,
                tmdbId: id,
                mediaType,
                preselectedSource: item.source,
                sourceAttemptsLog: Array.isArray(probe?.attempts) ? probe.attempts : [],
                sourceLastAttemptAt: now(),
                selectionLogId: runLogId,
                releaseDate: effectiveReleaseDate,
                ...
                seriesMeta: {
                  ...
                  episode: item.ep,
                  seasonEpisodeCount: episodeNums.length,
                  seasonTotalGb: totalGb,
                },
              });
              queuedCount += 1;
            }
            selectedSource = pickedEpisodes[0]?.source || null;
          }
        } else {
