...
function isSeasonPackName(name, seasonNumber = 1) {
  ...
  const sTag = `s${pad2(sn)}`;
  const hasSeason = s.includes(sTag) || s.includes(`season ${sn}`) || s.includes(`season.${sn}`) || s.includes(`season_${sn}`);
  if (!hasSeason) return false;
  // Exclude explicit single-episode patterns.
  if (/\bs\d{1,2}e\d{1,3}\b/.test(s) || /\b\d{1,2}x\d{1,3}\b/.test(s)) return false;
  // Exclude multi-season ranges like S01-S03 or S01E01-S02E10.
  const multi = s.match(/\bs(\d{1,2})\s*[-_]\s*s(\d{1,2})\b/);
  if (multi) {
    const end = Number(multi[2] || 0) || 0;
    if (end > sn) return false;
  }
  if (sn === 1 && /\bs0?2\b/.test(s)) return false;
  return true;
}
...
function seriesPipelineRuns({ settings, triggerReason = '' } = {}) {
  ...
  if (shouldUseDeferredSeriesPipeline(triggerReason)) {
    return pipelines.deferredRetry?.enabled === false
      ? []
      : [{ key: 'deferredRetry', mode: 'deferred_retry', pipeline: pipelines.deferredRetry }];
  }
  const runs = [];
  if (pipelines.newSeries?.enabled !== false) runs.push({ key: 'newSeries', mode: 'new_series_season_pack', pipeline: pipelines.newSeries });
  if (pipelines.newSeriesEpisode?.enabled !== false) {
    runs.push({ key: 'newSeriesEpisode', mode: 'new_series_episode', pipeline: pipelines.newSeriesEpisode });
  }
  if (pipelines.existingSeries?.enabled !== false) runs.push({ key: 'existingSeries', mode: 'existing_next_episode', pipeline: pipelines.existingSeries });
  return runs;
}
      ...
      for (const c of candidates) {
        ...
        if (t === 'series') {
          ...
          if (isEpisodeSelectionRun(selectionRun)) {
            ...
            const seasonPacks = pool.filter((row) => isSeasonPackName(row?.name || row?.title || '', 1));
            const bestPack = pickBest(seasonPacks);
