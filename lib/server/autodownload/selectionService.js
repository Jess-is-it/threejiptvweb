...
async function runSelectionJob({ type, force = false, preserveLastRun = false, targetTotal = null, triggerReason = '' } = {}) {
  const startedAt = now();
  const t = sanitizeType(type);
  ...
  try {
    ...
    return { ok: true, type: t, selected, log: logEntry };
  } catch (e) {
    ...
    return { ok: false, type: t, error: msg, selected, log: logEntry };
  } finally {
    if (ssh) await ssh.close();
  }
}

export async function runMovieSelectionJob({ force = false, preserveLastRun = false, targetTotal = null, triggerReason = '' } = {}) {
  return runSelectionJob({ type: 'movie', force, preserveLastRun, targetTotal, triggerReason });
}

export async function runSeriesSelectionJob({ force = false, preserveLastRun = false, targetTotal = null, triggerReason = '' } = {}) {
  return runSelectionJob({ type: 'series', force, preserveLastRun, targetTotal, triggerReason });
}
...
export async function runSelectionJobForType({
  ...
  const t = sanitizeType(type);
  return t === 'series'
    ? runSelectionJob({ type: 'series', force, preserveLastRun, targetTotal, triggerReason })
    : runSelectionJob({ type: 'movie', force, preserveLastRun, targetTotal, triggerReason });
}
