...
import { discoverMovies, discoverSeries, getTmdbDetailsById, getTmdbTvSeason } from './tmdbService';
...
import { normalizeSeriesPipelines } from './seriesPipelines';
...
async function getFinalSeriesEpisodeSet({ ssh, mountDir, title, year }) {
  const base = sanitizeFsName(`${title}${year ? ` (${year})` : ''}`);
  const baseNoYear = sanitizeFsName(`${title}`);
  const root = `${String(mountDir || '').replace(/\/+$/, '')}/Series`;
  const cmd = [
    ...
    `P1=${JSON.stringify(escapeFindGlob(base))}`,
    `P2=${JSON.stringify(escapeFindGlob(baseNoYear))}`,
    'MATCH="$(find "$ROOT" -maxdepth 4 -type d \\( -iname "$P1" -o -iname "$P2" \\) -print -quit 2>/dev/null || true)"',
    'if [ -z "$MATCH" ]; then exit 0; fi',
    'find "$MATCH" -type f \\( -iname "*.mkv" -o -iname "*.mp4" -o -iname "*.avi" -o -iname "*.mov" -o -iname "*.m4v" -o -iname "*.ts" -o -iname "*.webm" \\) -printf "%f\\n" 2>/dev/null || true',
  ].join('\n');
  const r = await ssh.exec(cmd, { timeoutMs: 60000 });
  const episodes = new Set();
  for (const name of String(r.stdout || '').split(/\r?\n/)) {
    const tag = parseEpisodeTag(name);
    if (!tag) continue;
    episodes.add(`${tag.season}:${tag.episode}`);
  }
  return episodes;
}

async function findNextMissingSeriesEpisode({ ssh, mountDir, tmdbId, details }) {
  const episodes = await getFinalSeriesEpisodeSet({
    ssh,
    mountDir,
    title: details?.title || '',
    year: details?.year || '',
  }).catch(() => new Set());
  if (!episodes.size) return null;

  const seasonLimit = Math.max(1, Math.min(10, Number(details?.numberOfSeasons || 1) || 1));
  for (let seasonNumber = 1; seasonNumber <= seasonLimit; seasonNumber += 1) {
    const season = await getTmdbTvSeason({ id: tmdbId, seasonNumber }).catch(() => null);
    const seasonEpisodes = Array.isArray(season?.episodes) ? season.episodes.filter(airedEpisode) : [];
    ...
    for (const episode of seasonEpisodes) {
      ...
      if (!episodes.has(`${seasonNumber}:${episodeNumber}`)) {
        return {
          season: seasonNumber,
...
async function findFirstSeasonEpisode({ tmdbId }) {
  const season = await getTmdbTvSeason({ id: tmdbId, seasonNumber: 1 }).catch(() => null);
  const episodes = Array.isArray(season?.episodes)
    ? season.episodes
        .filter(airedEpisode)
        .map((episode) => Number(episode?.episodeNumber || 0))
...
function isSelectionSourceValidationError(error) {
  const message = String(error?.message || '').trim();
  if (!message) return false;
  return /selected source does not match the tmdb title\/year\./i.test(message);
}
...
function qualityScore(quality) {
  ...
  if (q.includes('2160')) return 40;
  if (q.includes('1080')) return 30;
...
function scoreSourceRow(row) {
  const seed = Math.max(0, Number(row?.seeders || 0) || 0);
  const size = Number(row?.sizeGb || 0) || 0;
  // Bias toward seeders, then quality, then smaller size within reason.
  return seed * 10 + qualityScore(row?.quality) - Math.min(50, size);
}

function isSeasonPackName(name, seasonNumber = 1) {
  const s = String(name || '').toLowerCase().replace(/[._]+/g, ' ');
  const sn = Number(seasonNumber || 1) || 1;
  const season = String(sn);
  const seasonPadded = pad2(sn);
  const hasSeason =
    new RegExp(`\\bs0?${season}\\b`).test(s) ||
    new RegExp(`\\bseason\\s*0?${season}\\b`).test(s) ||
    new RegExp(`\\bcomplete\\s+season\\s*0?${season}\\b`).test(s);
  if (!hasSeason) return false;
  if (/\bs\d{1,2}\s*e\d{1,3}\b/.test(s) || /\b\d{1,2}x\d{1,3}\b/.test(s)) return false;
  if (/\bcomplete\s+(series|collection|show)\b/.test(s) || /\ball\s+seasons\b/.test(s)) return false;
  if (/\bs\d{1,2}\s*(?:-|to|through|thru|–|—)\s*s?\d{1,2}\b/.test(s)) return false;
  if (/\bseason\s*\d{1,2}\s*(?:-|to|through|thru|–|—)\s*\d{1,2}\b/.test(s)) return false;
  const seasons = [...s.matchAll(/\bs(\d{1,2})\b/g)].map((m) => Number(m[1])).filter(Boolean);
  if (seasons.some((n) => n !== sn)) return false;
  if (sn === 1 && new RegExp(`\\bs0?${Number(seasonPadded) + 1}\\b`).test(s)) return false;
  return true;
}

function isEpisodeName(name, seasonNumber = 1, episodeNumber = 1) {
  const s = String(name || '').toLowerCase();
  const sn = Number(seasonNumber || 1) || 1;
  const ep = Number(episodeNumber || 1) || 1;
  const re1 = new RegExp(`\\bs0?${sn}e0?${ep}\\b`, 'i');
  const re2 = new RegExp(`\\b${sn}x0?${ep}\\b`, 'i');
  const re3 = new RegExp(`\\bs${pad2(sn)}e${pad2(ep)}\\b`, 'i');
  return re1.test(s) || re2.test(s) || re3.test(s);
}

function pickBest(rows) {
  const list = Array.isArray(rows) ? rows : [];
  if (!list.length) return null;
  return [...list].sort((a, b) => scoreSourceRow(b) - scoreSourceRow(a))[0] || null;
}

function hasUsableSourceLink(row = {}) {
  return Boolean(String(row?.sourceUrl || row?.magnet || row?.url || row?.link || row?.hash || '').trim());
}

function pickBestSeasonPack(rows, { seasonNumber = 1, minSeeders = 0, maxSizeGb = null } = {}) {
  const minSeedersNum = Math.max(0, Math.floor(Number(minSeeders || 0) || 0));
  const maxSizeNum = maxSizeGb === null || maxSizeGb === undefined || String(maxSizeGb).trim() === '' ? null : Number(maxSizeGb);
  const seasonPacks = (Array.isArray(rows) ? rows : []).filter((row) => {
    const name = row?.name || row?.title || '';
    if (!isSeasonPackName(name, seasonNumber)) return false;
    if (!hasUsableSourceLink(row)) return false;
    const seeders = Math.max(0, Math.floor(Number(row?.seeders || 0) || 0));
    if (seeders < minSeedersNum) return false;
    const sizeGb = row?.sizeGb === null || row?.sizeGb === undefined ? NaN : Number(row.sizeGb);
    if (!Number.isFinite(sizeGb) || sizeGb <= 0) return false;
    if (maxSizeNum !== null && Number.isFinite(maxSizeNum) && maxSizeNum > 0 && sizeGb > maxSizeNum) return false;
    return true;
  });
  return pickBest(seasonPacks);
}

function uniqueSourceCandidates(rows = []) {
  const seen = new Set();
  const out = [];
  for (const row of Array.isArray(rows) ? rows : []) {
    const key = String(row?.hash || row?.magnet || row?.sourceUrl || row?.name || '').trim().toLowerCase();
...
function parseEpisodeTag(name) {
  const s = String(name || '').toLowerCase();
  const m = s.match(/\bs(\d{1,2})e(\d{1,3})\b/);
  if (m?.[1] && m?.[2]) {
    const season = Number(m[1]);
    const episode = Number(m[2]);
    if (Number.isFinite(season) && season > 0 && Number.isFinite(episode) && episode > 0) return { season, episode };
  }
  const x = s.match(/\b(\d{1,2})x(\d{1,3})\b/);
  if (x?.[1] && x?.[2]) {
    const season = Number(x[1]);
    const episode = Number(x[2]);
    if (Number.isFinite(season) && season > 0 && Number.isFinite(episode) && episode > 0) return { season, episode };
  }
  return null;
}

function airedEpisode(episode = {}) {
  const raw = String(episode?.airDate || '').trim();
  if (!raw) return true;
...
function normalizeStrategyFromPipeline(pipeline = {}) {
  const strategy = pipeline?.strategy && typeof pipeline.strategy === 'object' ? pipeline.strategy : {};
  return {
    recentMonthsRange: Math.max(1, Number(strategy.recentMonthsRange || 12) || 12),
    classicYearStart: Number(strategy.classicYearStart || 2008) || 2008,
    classicYearEnd: Number(strategy.classicYearEnd || 2020) || 2020,
    counts: {
      recentAnimation: Math.max(0, Number(strategy.recentAnimationCount || 0) || 0),
      recentLive: Math.max(0, Number(strategy.recentLiveActionCount || 0) || 0),
      classicAnimation: Math.max(0, Number(strategy.classicAnimationCount || 0) || 0),
      classicLive: Math.max(0, Number(strategy.classicLiveActionCount || 0) || 0),
    },
  };
}

function shouldUseDeferredSeriesPipeline(triggerReason = '') {
  return /timeout|replacement|deferred|retry/i.test(String(triggerReason || ''));
}

function seriesPipelineRuns({ settings, triggerReason = '' } = {}) {
  const pipelines = normalizeSeriesPipelines(settings || {});
  if (shouldUseDeferredSeriesPipeline(triggerReason)) {
    return pipelines.deferredRetry?.enabled === false
      ? []
      : [{ key: 'deferredRetry', mode: 'series_tv_pack_retry', pipeline: pipelines.deferredRetry }];
  }
  return pipelines.newSeries?.enabled === false
    ? []
    : [{ key: 'newSeries', mode: 'series_tv_pack', pipeline: pipelines.newSeries }];
}

function isExistingSeriesContinuationRun(selectionRun) {
  return selectionRun?.mode === 'existing_next_episode';
}

function isNewSeriesEpisodeRun(selectionRun) {
  return selectionRun?.mode === 'new_series_episode';
}

function isEpisodeSelectionRun(selectionRun) {
  return isExistingSeriesContinuationRun(selectionRun) || isNewSeriesEpisodeRun(selectionRun);
}

function shouldRememberSkippedDuplicate(type, selectionRun) {
  if (String(type || '').trim().toLowerCase() !== 'series') return true;
  return isExistingSeriesContinuationRun(selectionRun);
}
...
async function runSelectionJob({
  ...
  const selectionRuns =
    t === 'series'
      ? seriesPipelineRuns({ settings, triggerReason }).map((run) => ({
          ...run,
          buckets: buildBuckets({ type: t, ...normalizeStrategyFromPipeline(run.pipeline) }),
        }))
      : [{ key: 'default', mode: 'default', pipeline: null, buckets: buildBuckets({ type: t, ...normalizeStrategy(t, settings) }) }];
  ...
  const runLogId = appendToLogId || crypto.randomUUID();
  const selected = [];
  const pickedCounts = {
    recentAnimationSelected: 0,
    recentLiveActionSelected: 0,
  ...
  try {
    ...
    for (const selectionRun of selectionRuns) {
      if (cappedTarget !== null && selected.length >= cappedTarget) break;
      const runBuckets = Array.isArray(selectionRun?.buckets) ? selectionRun.buckets : [];
      for (const b of runBuckets) {
        if (cappedTarget !== null && selected.length >= cappedTarget) break;
        if (!b.count) continue;
      const rememberSkippedDuplicate = shouldRememberSkippedDuplicate(t, selectionRun);
      ...
      candidates = candidates.filter((c) => {
        ...
        const allowExistingSeries = t === 'series' && isExistingSeriesContinuationRun(selectionRun);
        if (!allowExistingSeries && xuiCatalogHasTmdbId({ catalog: xuiCatalog, type: t, tmdbId: id })) {
          if (rememberSkippedDuplicate) dedupeIds.add(id);
          return false;
        }
        const { title, year } = discoverItemTitleAndYear(mediaType, c);
        if (!allowExistingSeries && title && inventoryIndexHas({ index: inventoryIndex, title, year })) {
          // Skip items already present on NAS so the selector focuses on new titles.
          if (rememberSkippedDuplicate) dedupeIds.add(id);
          return false;
        }
        return true;
      ...
      for (const c of candidates) {
        ...
        if (
          !(t === 'series' && isExistingSeriesContinuationRun(selectionRun)) &&
          !checkedFinalLibrary &&
          hasInventoryMatch({
            inventory,
            type: t,
        ...
        if (t === 'series') {
          const pipeline = selectionRun?.pipeline || {};
          const imdbQuery = String(details?.imdbId || '').trim();
          if (!imdbQuery) {
            skippedNoSourceCount++;
            continue;
          }

          if (isEpisodeSelectionRun(selectionRun)) {
            const targetEpisode = isExistingSeriesContinuationRun(selectionRun)
              ? ssh
                ? await findNextMissingSeriesEpisode({ ssh, mountDir, tmdbId: id, details }).catch(() => null)
                : null
              : await findFirstSeasonEpisode({ tmdbId: id }).catch(() => null);
            if (!targetEpisode) {
              skippedNoSourceCount++;
              continue;
            }
            const acquisitionMode =
              pipeline.acquisitionMode || (isNewSeriesEpisodeRun(selectionRun) ? 'first_episode' : 'next_episode');
            const probe = await searchBestDownloadSource({
              query: imdbQuery,
              year: null,
              type: 'series',
              episodeHint: { season: targetEpisode.season, episode: targetEpisode.episode },
              selectionLogId: runLogId,
              correlationId: `selection-${t}-${selectionRun.key}-${startedAt}-${id}`,
              jobId: `selection-${t}-${selectionRun.key}-${id}`,
              stopOnFirstValid: false,
              minSeeders: pipeline.minSeeders,
              maxSizeGb: pipeline.maxEpisodeGb,
              maxResults: 50,
            }).catch((e) => ({ ok: false, error: e?.message || 'Source lookup failed.' }));
            selectedSource = pickBest(Array.isArray(probe?.candidates) ? probe.candidates : []);
            ...
            try {
              await addQueueItemFromTmdb({
                ...
                sourceAttemptsLog: Array.isArray(probe?.attempts) ? probe.attempts : [],
                ...
                seriesMeta: {
                  mode: 'episode',
                  pipelineKey: selectionRun.key,
                  acquisitionMode,
                  season: targetEpisode.season,
                  episode: targetEpisode.episode,
                  seasonEpisodeCount: targetEpisode.seasonEpisodeCount,
                  timeoutHours: pipeline.timeoutHours,
                  minSeeders: pipeline.minSeeders,
                  maxEpisodeGb: pipeline.maxEpisodeGb,
                  maxSeasonTotalGb: pipeline.maxSeasonTotalGb,
                },
              });
            } catch (error) {
              if (isSelectionSourceValidationError(error)) {
                skippedNoSourceCount++;
                selectedSource = null;
                continue;
              }
            ...
            const season1 = await getTmdbTvSeason({ id, seasonNumber: 1 }).catch(() => null);
            const episodeNums = Array.isArray(season1?.episodes) ? season1.episodes.map((e) => Number(e?.episodeNumber || 0)).filter((n) => n > 0) : [];
            if (!episodeNums.length) {
              skippedNoSourceCount++;
              continue;
            }

            const maxSeasonTotalGb = Number(pipeline.maxSeasonTotalGb || 0) > 0 ? Number(pipeline.maxSeasonTotalGb) : null;
            const sourceQueries = uniqueStrings([
              imdbQuery,
              details?.title ? `${details.title} S01` : '',
              details?.title ? `${details.title} Season 1` : '',
            ]);
            let probe = { ok: false, candidates: [], attempts: [], error: '' };
            for (const sourceQuery of sourceQueries) {
              const nextProbe = await searchBestDownloadSource({
                query: sourceQuery,
                year: null,
                type: 'series',
                selectionLogId: runLogId,
                ...
                stopOnFirstValid: false,
                minSeeders: pipeline.minSeeders,
                maxSizeGb: maxSeasonTotalGb,
                maxResults: 75,
              }).catch((e) => ({ ok: false, error: e?.message || 'Source lookup failed.', candidates: [], attempts: [] }));
              probe = {
                ok: Boolean(probe.ok || nextProbe?.ok),
                error: nextProbe?.error || probe.error || '',
                candidates: uniqueSourceCandidates([...(probe.candidates || []), ...(Array.isArray(nextProbe?.candidates) ? nextProbe.candidates : [])]),
                attempts: [...(probe.attempts || []), ...(Array.isArray(nextProbe?.attempts) ? nextProbe.attempts : [])],
              };
              const earlyPack = pickBestSeasonPack(probe.candidates, {
                seasonNumber: 1,
                minSeeders: pipeline.minSeeders,
                maxSizeGb: maxSeasonTotalGb,
              });
            ...
            const pool = Array.isArray(probe?.candidates) ? probe.candidates : [];
            if (!pool.length) {
              skippedNoSourceCount++;
              continue;
            }

            const bestPack = pickBestSeasonPack(pool, {
              seasonNumber: 1,
              minSeeders: pipeline.minSeeders,
              maxSizeGb: maxSeasonTotalGb,
            });
            const packGb = bestPack?.sizeGb === null || bestPack?.sizeGb === undefined ? NaN : Number(bestPack.sizeGb);
            const packOk = bestPack && Number.isFinite(packGb) && packGb > 0 && (maxSeasonTotalGb === null || packGb <= maxSeasonTotalGb);

            if (packOk) {
              selectedSource = bestPack;
              try {
                await addQueueItemFromTmdb({
                  type: t,
                  tmdbId: id,
                  mediaType,
                  preselectedSource: bestPack,
                  sourceAttemptsLog: Array.isArray(probe?.attempts) ? probe.attempts : [],
                  sourceLastAttemptAt: now(),
                  selectionLogId: runLogId,
                  releaseDate: effectiveReleaseDate,
                  ...
                  seriesMeta: {
                    mode: 'season_pack',
                    pipelineKey: selectionRun.key,
                    acquisitionMode: pipeline.acquisitionMode || 'season_pack',
                    season: 1,
                    episode: null,
                    seasonEpisodeCount: episodeNums.length,
                    seasonTotalGb: packGb,
                    timeoutHours: pipeline.timeoutHours,
                    minSeeders: pipeline.minSeeders,
                    maxEpisodeGb: pipeline.maxEpisodeGb,
                    maxSeasonTotalGb: pipeline.maxSeasonTotalGb,
          ...
          selectedSource = sourceProbe?.selected || null;
          const sourceUrl = String(selectedSource?.sourceUrl || selectedSource?.magnet || '').trim();
          if (!sourceUrl) {
            skippedNoSourceCount++;
            continue;
        ...
        selected.push({
          ...
          pipelineKey: t === 'series' ? String(selectionRun?.key || '').trim() : '',
          acquisitionMode: t === 'series' ? String(selectionRun?.pipeline?.acquisitionMode || '').trim() : '',
          queuedCount,
          selectionLogId: runLogId,
          releaseDate: effectiveReleaseDate,
          releaseTag: effectiveReleaseTag,
      ...
      if (got < b.count) {
        insufficient.push({
          bucket: t === 'series' ? `${selectionRun.key}:${b.key}` : b.key,
          requested: b.count,
          selected: got,
        });
      }
