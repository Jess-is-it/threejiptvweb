...
export function normalizeSeriesPipelines(settings = {}) {
  ...
  const newSeries = normalizePipeline(saved.newSeries, newDefaults, legacyStrategy);
  const newSeriesEpisode = {
    ...normalizePipeline(saved.newSeriesEpisode, newEpisodeDefaults, legacyStrategy),
    enabled: false,
    acquisitionMode: DEFAULT_SERIES_PIPELINES.newSeriesEpisode.acquisitionMode,
  };
  const existingSeries = {
    ...normalizePipeline(saved.existingSeries, DEFAULT_SERIES_PIPELINES.existingSeries, DEFAULT_SERIES_PIPELINES.existingSeries.strategy),
    enabled: false,
    acquisitionMode: DEFAULT_SERIES_PIPELINES.existingSeries.acquisitionMode,
  };
  const deferredRetry = {
    ...normalizePipeline(
      saved.deferredRetry,
      {
        ...DEFAULT_SERIES_PIPELINES.deferredRetry,
        minSeeders: newSeries.minSeeders,
        maxSeasonTotalGb: newSeries.maxSeasonTotalGb,
        timeoutHours: newSeries.timeoutHours,
        strategy: newSeries.strategy,
      },
      newSeries.strategy
    ),
    enabled: saved?.deferredRetry?.enabled === false ? false : true,
    acquisitionMode: 'season_pack',
    minSeeders: newSeries.minSeeders,
    maxEpisodeGb: newSeries.maxEpisodeGb,
    maxSeasonTotalGb: newSeries.maxSeasonTotalGb,
    timeoutHours: newSeries.timeoutHours,
    strategy: newSeries.strategy,
  };
