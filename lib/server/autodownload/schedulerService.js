...
export async function schedulerTick({ force = false, type = 'all' } = {}) {
  ...
  out.mountGuard = mountReady
    ? await resumeManagedDownloadsPausedForMount().catch((e) => ({ ok: false, action: 'resume', error: e?.message || 'resume failed' }))
    : await pauseManagedDownloadsForMountOffline().catch((e) => ({ ok: false, action: 'pause', error: e?.message || 'pause failed' }));

  out.deletion = await runDeletionCycle().catch((e) => ({
    ok: false,
  ...
  const dispatchableCounts = await countDispatchableDownloads(dispatchType, out.at).catch(() => ({ movie: 0, series: 0 }));
  ...
  out.timeouts = mountBlocksDownloads
    ...
    : await runTimeoutChecker().catch((e) => ({ ok: false, error: e?.message || 'timeout checker failed' }));
  ...
  out.release = mountBlocksDownloads
    ? { ok: true, skipped: true, reason: 'mount_not_ready' }
    : await releaseDueSelections({ type: dispatchType }).catch((e) => ({
        ok: false,
