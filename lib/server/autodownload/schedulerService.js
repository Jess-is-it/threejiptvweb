...
import { startQueuedDownloads, syncDownloadsFromClient, runTimeoutChecker } from './downloadService';
...
export async function schedulerTick({ force = false, type = 'all' } = {}) {
  ...
  if (shouldDispatch) {
    out.dispatch = await startQueuedDownloads({ type: dispatchType, limitPerType: 2, limitByType }).catch((e) => ({
      ok: false,
  ...
  out.processed = await processNextCompleted({ type: dispatchType }).catch((e) => ({ ok: false, error: e?.message || 'processing failed' }));
