...
import { clearXuiMediaIndexCache, getCachedXuiMediaIndex, getXuiMediaIndex, matchXuiMedia } from './xuiMediaIndexService';
...
const DELETION_FALLBACK_HYDRATE_LIMIT = 150;
...
function deletionLog(message, extra = null) {
  ...
  console.log(`[deletion] ${message} heap=${heapMb}MB${suffix}`);
}

async function runDeletionPhase(name, fn) {
  const startedAt = now();
  deletionLog(`${name}:start`);
  try {
    const result = await fn();
    deletionLog(`${name}:done`, { durationMs: now() - startedAt });
    return result;
  } catch (error) {
    deletionLog(`${name}:error`, { durationMs: now() - startedAt, error: error?.message || String(error) });
    throw error;
  }
}
...
async function mapWithConcurrency(items = [], limit = 4, mapper = async (item) => item) {
  const rows = Array.isArray(items) ? items : [];
  const concurrency = Math.max(1, Math.min(8, Math.floor(Number(limit || 4) || 4)));
...
async function ensureDeletionPreview({
  ...
  try {
    const xuiIndex =
      getCachedXuiMediaIndex({ maxAgeMs: 5 * 60 * 1000 }) ||
      (await runDeletionPhase('preview:xuiIndex', () =>
        getXuiMediaIndex({ maxAgeMs: 5 * 60 * 1000, force: false }).catch(() => null)
      ));
    const watchMap = await runDeletionPhase('preview:watchMap', () =>
      getRecentWatchMap({
        watchWindowDays: Number(deletionSettings?.protectRecentWatchDays || 0) || 7,
        limit: 5000,
      }).catch(() => ({ movieMap: new Map(), seriesMap: new Map() }))
    );
    const { all, seriesCount } = await runDeletionPhase('preview:collectCandidates', () =>
      collectCandidates({ db, settings, watchMap, xuiIndex })
    );
    ...
    try {
      ...
      await runDeletionPhase('preview:storageStats', () =>
        enrichCandidatesWithStorage({ candidates: all, ssh: connectedSsh, vodStorage, nasOnline })
      );
    } finally {
    ...
    const fallbackPool = usedFallbackBypass ? actionableCandidates.slice(0, DELETION_FALLBACK_HYDRATE_LIMIT) : actionableCandidates;
    const fallbackCandidates = usedFallbackBypass
      ? await runDeletionPhase('preview:releaseDates', () => hydrateCandidateReleaseDates(fallbackPool))
      : actionableCandidates;
    const selected = await runDeletionPhase('preview:visuals', () =>
      hydrateCandidateVisuals(
        usedFallbackBypass
...
export async function runDeletionCycle() {
  const db = await runDeletionPhase('cycle:loadDb', () => getAdminDb());
  const settings = db?.autodownloadSettings || (await runDeletionPhase('cycle:settings', () => getAutodownloadSettings()));
  const mountStatus = db?.mountStatus?.space?.total
    ? db.mountStatus
    : await runDeletionPhase('cycle:mountStatus', () => fetchMountStatus().catch(() => null));
  const vodStorage = await runDeletionPhase('cycle:vodStorage', () => fetchVodStorageDevices().catch(() => null));
  const scheduled = await runDeletionPhase('cycle:schedule', () => scheduleDeletionLogs({ db, mountStatus, vodStorage, settings }));
  const refreshedDb = await runDeletionPhase('cycle:reloadDbBeforeExecute', () => getAdminDb());
  const executed = await runDeletionPhase('cycle:executeDue', () => executeDueDeletionLogs({ db: refreshedDb, mountStatus }));
  const finalDb = await runDeletionPhase('cycle:finalDb', () => getAdminDb());
...
export async function getDeletionLogs({ type = 'all', limit = 200, refreshPreview = false } = {}) {
  const db = await getAdminDb();
  const settings = db?.autodownloadSettings || (await getAutodownloadSettings());
  const currentPreview = db?.deletionPreview || null;
  const previewReusable = canReusePreviewForPageLoad(currentPreview, settings);
  if (refreshPreview) {
  ...
  const refreshedDb = await getAdminDb();
  const refreshedPreview = refreshedDb?.deletionPreview || null;
  const refreshedPreviewReusable = canReusePreviewForPageLoad(refreshedPreview, refreshedDb?.autodownloadSettings || settings);
  ...
  return {
    ...
    previewReusable: refreshedPreviewReusable,
    previewStale: Boolean(refreshedPreview && !refreshedPreviewReusable),
    previewRefreshSkipped: !refreshPreview && !previewReusable,
  };
}
