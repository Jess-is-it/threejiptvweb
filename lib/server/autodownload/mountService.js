...
const STORAGE_DEVICES_CACHE_TTL_MS = 60 * 1000;
let storageDevicesCache = {
  at: 0,
  key: '',
  value: null,
  promise: null,
...
function sanitizeAbsolutePath(dir, { fallback = '' } = {}) {
  const d = String(dir || '').trim() || fallback;
  if (!d) return '';
  if (!d.startsWith('/')) throw new Error('Path must be an absolute path.');
  return d.replace(/\/+$/, '');
}

function sanitizeShare(s) {
  const share = String(s || '')
    .trim()
    .replace(/\\/g, '/')
    .replace(/^\/+/, '')
    .replace(/\/+$/, '')
  ...
  const parts = share.split('/').filter(Boolean);
  if (parts.some((part) => part === '.' || part === '..')) {
    throw new Error('Share path cannot contain . or .. segments.');
  }
  if (parts.some((part) => /[\0\r\n,]/.test(part))) {
    throw new Error('Share path cannot contain commas or control characters.');
  }
  return parts.join('/');
}

function buildCifsTarget(windowsHost, shareName) {
  const winHost = sanitizeHost(windowsHost);
  const sharePath = sanitizeShare(shareName);
  const [share, ...prefixParts] = sharePath.split('/');
  const prefixPath = prefixParts.join('/');
  const source = `//${winHost}/${sharePath}`;
  const displaySource = source;
  return { winHost, share, sharePath, prefixPath, source, displaySource };
}
...
export async function upsertMountSettingsFromAdminInput(input) {
  ...
  const xuiVodPath = sanitizeAbsolutePath(input?.xuiVodPath, { fallback: '/home/xui/content/vod' });
...
export async function updateMountSettingsFromAdminInput(input) {
  ...
  const xuiVodPath = sanitizeAbsolutePath(input?.xuiVodPath ?? prev?.xuiVodPath, { fallback: '/home/xui/content/vod' });
...
function readTaggedSection(text, startTag, endTag = '') {
  const source = String(text || '');
  const start = source.indexOf(startTag);
  if (start < 0) return '';
  const from = start + startTag.length;
  const next = endTag ? source.indexOf(endTag, from) : -1;
  const chunk = next >= 0 ? source.slice(from, next) : source.slice(from);
...
function parseDfLine(line) {
  const parts = String(line || '')
    ...
    .split(/\s+/)
    .filter(Boolean);
  if (parts.length < 3) return { size: 0, used: 0, avail: 0, target: '' };
  return {
    size: Number(parts[0]) || 0,
    used: Number(parts[1]) || 0,
    avail: Number(parts[2]) || 0,
...
function blockSourceAliases(source) {
  ...
  if (basename) {
    ...
    aliases.add(`/dev/${basename}`);
    if (basename.startsWith('mapper/')) {
      aliases.add(`/dev/${basename}`);
      aliases.add(basename.replace(/^mapper\//, ''));
    }
  }
  return aliases;
}

function blockNodeMatchesSource(node, source) {
  if (!node || !source) return false;
  const aliases = blockSourceAliases(source);
  return aliases.has(String(node.path || '').trim()) || aliases.has(String(node.kname || '').trim()) || aliases.has(String(node.name || '').trim());
}

function collectBlockLineages(nodes, source, trail = [], out = []) {
  for (const node of Array.isArray(nodes) ? nodes : []) {
    const nextTrail = [...trail, node];
    if (blockNodeMatchesSource(node, source)) out.push(nextTrail);
    if (Array.isArray(node.children) && node.children.length) {
      collectBlockLineages(node.children, source, nextTrail, out);
    }
  }
  return out;
}

function summarizeBlockNode(node) {
  if (!node) return null;
  return {
    name: String(node.name || ''),
    kname: String(node.kname || ''),
    path: String(node.path || ''),
    type: String(node.type || ''),
    fstype: String(node.fstype || ''),
...
function uniqueByPath(nodes) {
  ...
  for (const node of nodes) {
    const key = String(node?.path || node?.name || '').trim();
...
function detectPoolType(source, logicalNode, memberNodes) {
  const normalized = normalizeBlockSource(source);
  const logicalType = String(logicalNode?.type || '').toLowerCase();
  if (!normalized) return 'unknown';
  if (!normalized.startsWith('/dev/')) return 'virtual';
  if (logicalType === 'lvm') return 'lvm';
  if (logicalType === 'raid') return 'raid';
  if (logicalType === 'crypt') return 'encrypted';
  if (memberNodes.length > 1) return 'combined';
  return 'direct';
}

function buildStorageDevicesSummary({ preferredPath, resolvedPath, pathExists, candidatePaths, findmnt, df, lsblk }) {
  const filesystem = Array.isArray(findmnt?.filesystems) ? findmnt.filesystems[0] || null : null;
  const logicalSource = normalizeBlockSource(filesystem?.source || '');
  const logicalTarget = String(filesystem?.target || resolvedPath || preferredPath || '');
  const logicalFstype = String(filesystem?.fstype || '');
  const blockdevices = Array.isArray(lsblk?.blockdevices) ? lsblk.blockdevices : [];
  const lineages = logicalSource ? collectBlockLineages(blockdevices, logicalSource) : [];
  const logicalNode = lineages.length ? lineages[0][lineages[0].length - 1] : null;
  const backingNodes = uniqueByPath(
    lineages
      .flatMap((lineage) => lineage.slice(0, -1))
      .filter(Boolean)
  );
  const memberDisks = uniqueByPath(backingNodes.filter((node) => String(node?.type || '') === 'disk'));
  const poolType = detectPoolType(logicalSource, logicalNode, memberDisks);
  const pooled = poolType !== 'direct';
  const logicalSize = Number(df?.size) || 0;
  const logicalUsed = Number(df?.used) || 0;
  const logicalAvail = Number(df?.avail) || 0;
  const logicalUsedPct = logicalSize > 0 ? Math.round((logicalUsed / logicalSize) * 100) : null;

  let note = '';
  if (!pathExists) {
    note = 'Default XUI VOD path was not found on the engine host. This panel is display-only and depends on detected directories.';
  } else if (poolType === 'combined' || poolType === 'lvm' || poolType === 'raid') {
    note = 'This storage appears to be combined into one logical volume. Free space is reliable at the mounted volume level, not per physical disk.';
  } else if (poolType === 'virtual') {
    note = 'The mounted source is not a direct block device. Free space is shown for the resolved volume only.';
  } else if (!logicalSource) {
    note = 'Mounted volume was detected, but its backing block device could not be resolved.';
  }

  const backingRows = backingNodes.map((node) => {
    const summary = summarizeBlockNode(node);
    const nodeType = String(summary?.type || '');
    const role =
      nodeType === 'disk' ? 'Physical disk' : nodeType === 'part' ? 'Backing partition' : 'Backing device';
    return {
      ...summary,
      role,
      available: null,
      ...
      used: null,
      usedPct: null,
      note: pooled ? 'Part of combined storage pool.' : 'Backing device for the active volume.',
    };
  });

  const logicalRow = logicalSource
    ? {
        ...summarizeBlockNode(logicalNode),
        path: logicalSource,
        type: summarizeBlockNode(logicalNode)?.type || 'mount',
        fstype: logicalFstype || summarizeBlockNode(logicalNode)?.fstype || '',
        mountpoint: logicalTarget,
        size: logicalSize,
        role: 'Active volume',
        available: logicalAvail,
        availableKnown: logicalAvail > 0 || logicalSize > 0,
        used: logicalUsed,
        usedPct: logicalUsedPct,
        note:
          poolType === 'direct'
            ? 'This is the active mounted volume for XUI VOD.'
            : 'This is the mounted logical volume that aggregates the backing devices below.',
      }
    : null;
  ...
  return {
    ...
    rows: [logicalRow, ...backingRows].filter(Boolean),
    memberDiskCount: memberDisks.length,
    memberDiskRawTotal: memberDisks.reduce((sum, node) => sum + (Number(node?.size) || 0), 0),
  };
}
...
export async function testSmbAccess({ settingsOverride = null } = {}) {
  ...
  const target = buildCifsTarget(s.windowsHost, s.shareName);

  const script = `
set -euo pipefail

WIN_HOST=${JSON.stringify(target.winHost)}
CIFS_SOURCE=${JSON.stringify(target.source)}
DISPLAY_SOURCE=${JSON.stringify(target.displaySource)}
...
echo "Testing mount source: $DISPLAY_SOURCE"

if ! timeout 5 bash -c 'cat < /dev/null > /dev/tcp/$1/445' _ "$WIN_HOST" >/dev/null 2>&1; then
  echo "NAS host $WIN_HOST is not reachable on TCP 445 from the Engine Host."
  exit 1
fi
...
if ! timeout 45 mount -t cifs "$CIFS_SOURCE" "$MOUNT_DIR" -o "credentials=$CREDS,iocharset=utf8,file_mode=0775,dir_mode=0775,uid=$uid_num,gid=$gid_num${versOpt}"; then
  echo ""
  echo "Mount failed. Common causes:"
  echo " - Share path is wrong (the first segment must be the SMB share name)"
  echo " - Host is wrong/unreachable"
  echo " - Credentials are wrong or do not have permission"
  echo ""
  echo "Attempted: $DISPLAY_SOURCE -> $MOUNT_DIR"
  exit 1
fi

timeout 8 bash -c 'touch "$1" && rm -f "$1"' _ "$MOUNT_DIR/.3jtv_write_test"
echo "OK"
`;
...
FSTAB_LINE="$CIFS_SOURCE $MOUNT_DIR cifs $MOUNT_OPTS,_netdev,nofail 0 0"
...
EXPECTED_SOURCE="\${3:-${target.source}}"

ts() { date +%s%3N; }
now="$(ts)"
NAS_HOST="$(printf '%s' "$EXPECTED_SOURCE" | sed 's#^//##; s#/.*##')"
...
tcp445_ok() {
  timeout 5 bash -c 'cat < /dev/null > /dev/tcp/$1/445' _ "$NAS_HOST" >/dev/null 2>&1
}

write_test() {
  local test_file="$MOUNT_DIR/.3jtv_write_test"
  timeout 8 bash -c 'touch "$1" && rm -f "$1"' _ "$test_file" >/dev/null 2>&1
}
...
if ! tcp445_ok; then
  network_ok=false
  error="NAS host $NAS_HOST is not reachable on TCP 445"
fi

if [ -n "$source" ] && [ "$source" = "$EXPECTED_SOURCE" ]; then
  mounted=true
  if [ "$network_ok" = "true" ] && write_test; then
    writable=true
  elif [ "$network_ok" = "true" ]; then
    error="Mount not writable"
  fi
else
  if [ "$network_ok" = "true" ]; then error="Not mounted"; fi
fi

if [ "$mounted" != "true" ] || [ "$writable" != "true" ]; then
  if [ "$network_ok" = "true" ]; then
    timeout 45 mount -a >/dev/null 2>&1 || true
    timeout 45 mount "$MOUNT_DIR" >/dev/null 2>&1 || true
  fi
  source="$(current_source)"
  if [ -n "$source" ] && [ "$source" = "$EXPECTED_SOURCE" ]; then
    mounted=true
    ...
    mounted=false
    writable=false
    if [ "$network_ok" = "true" ]; then error="Not mounted (after remount)"; fi
  fi
fi

df_json="{}"
if [ "$mounted" = "true" ] && [ "$network_ok" = "true" ] && command -v df >/dev/null 2>&1; then
  # bytes
  df_out="$(timeout 8 df -B1 "$MOUNT_DIR" 2>/dev/null | tail -n 1 || true)"
  if [ -n "$df_out" ]; then
...
mount_out="$(timeout 45 mount -t cifs "$EXPECTED_SOURCE" "$MOUNT_DIR" -o "$MOUNT_OPTS" 2>&1)" || true
...
if [ -z "$actual_source" ] || [ "$actual_source" != "$EXPECTED_SOURCE" ]; then
  ...
  echo "Attempted source: $DISPLAY_SOURCE"
...
export async function fetchMountStatus({ ssh: injectedSsh = null } = {}) {
  ...
  const mountDir = sanitizeMountDir(s.mountDir);
  const target = buildCifsTarget(s.windowsHost, s.shareName);
  const nasHost = target.winHost;
  const expectedSource = target.source.toLowerCase();
  const smbVersion = sanitizeSmbVersion(s.smbVersion || '');
  const uid = sanitizeUidGid(s.uid, 'xui');
  const gid = sanitizeUidGid(s.gid, 'xui');
  const versOpt = smbVersion ? `,vers=${smbVersion}` : '';
...
MOUNT_DIR=${JSON.stringify(mountDir)}
NAS_HOST=${JSON.stringify(nasHost)}
...
tcp445_ok=true
tcp445_error=""
if ! timeout 5 bash -c 'cat < /dev/null > "/dev/tcp/$1/445"' _ "$NAS_HOST" >/dev/null 2>&1; then
  tcp445_ok=false
  tcp445_error="NAS host $NAS_HOST is not reachable on TCP 445"
fi
...
if [ "$mounted" != "true" ] && [ "$fstab_present" = "true" ]; then
  if [ "$tcp445_ok" = "true" ]; then
    mount_err="$(mkdir -p "$MOUNT_DIR" >/dev/null 2>&1; timeout 45 mount -t cifs "$EXPECTED_SOURCE" "$MOUNT_DIR" -o "$OPTS" 2>&1 || true)"
  else
    mount_err="$tcp445_error"
  fi
  source="$(read_source)"
  source_lc="$(printf '%s' "$source" | tr '[:upper:]' '[:lower:]')"
  if [ -n "$source" ] && { [ "$source_lc" = "$EXPECTED_SOURCE" ] || printf '%s' "$source_lc" | grep -q '^//'; }; then
...
if [ "$mounted" = "true" ]; then
  ...
  if [ "$tcp445_ok" != "true" ]; then
    err_msg="$tcp445_error"
  elif timeout 8 bash -c 'touch "$1" && rm -f "$1"' _ "$test_file" >/dev/null 2>&1; then
    writable=true
    err_msg=""
  else
...
avail=0
if [ "$mounted" = "true" ]; then
  set -- $(timeout 8 df -B1 "$MOUNT_DIR" 2>/dev/null | tail -n 1 || true)
  total="\${2:-0}"
  used="\${3:-0}"
  avail="\${4:-0}"
fi
...
`;
    ...
    const probe = await ssh.execScript(probeScript, { sudo: true, timeoutMs: 90000 });
...
export async function fetchVodStorageDevices({ ssh: injectedSsh = null } = {}) {
  const engineHost = await getEngineHost();
  if (!engineHost) throw new Error('No Engine Host configured.');

  const mountSettings = await getMountSettings();
  const preferredPath = sanitizeAbsolutePath(mountSettings?.xuiVodPath, { fallback: '/home/xui/content/vod' });
  const cacheKey = `${engineHost.id || engineHost.host}:${preferredPath}`;
  if (!injectedSsh) {
    if (
      storageDevicesCache.key === cacheKey &&
      storageDevicesCache.value &&
      now() - Number(storageDevicesCache.at || 0) < STORAGE_DEVICES_CACHE_TTL_MS
    ) {
      return storageDevicesCache.value;
    }
    if (storageDevicesCache.key === cacheKey && storageDevicesCache.promise) {
      return storageDevicesCache.promise;
    }
  }
  const ssh = injectedSsh || sshFromEngineHost(engineHost);
  const owns = !injectedSsh;
...
PREFERRED=${JSON.stringify(preferredPath)}

collect_candidates() {
  {
    [ -d "$PREFERRED" ] && printf '%s\n' "$PREFERRED" || true
    for pattern in /home/*/content/vod /home/*/vod /mnt/*/vod; do
...
RESOLVED="$PREFERRED"
PATH_EXISTS=false
if [ -d "$RESOLVED" ] || findmnt -T "$RESOLVED" >/dev/null 2>&1; then
  PATH_EXISTS=true
else
  RESOLVED="$(printf '%s\n' "$CANDIDATES" | head -n 1 || true)"
  if [ -n "$RESOLVED" ] && { [ -d "$RESOLVED" ] || findmnt -T "$RESOLVED" >/dev/null 2>&1; }; then
    PATH_EXISTS=true
  else
    RESOLVED="$PREFERRED"
  fi
...
if [ "$PATH_EXISTS" = "true" ]; then
  FINDMNT_JSON="$(findmnt -T "$RESOLVED" -J -b -o TARGET,SOURCE,FSTYPE 2>/dev/null | tr '\n' ' ' || true)"
  DF_LINE="$(df -B1 --output=size,used,avail,target "$RESOLVED" 2>/dev/null | tail -n 1 | tr -s ' ' || true)"
  SOURCE_DEVICE="$(findmnt -T "$RESOLVED" -n -o SOURCE 2>/dev/null | head -n 1 || true)"
  if [ -n "$SOURCE_DEVICE" ] && [ -e "$SOURCE_DEVICE" ]; then
    ROOT_DEVICE="$(lsblk -no PKNAME "$SOURCE_DEVICE" 2>/dev/null | tail -n 1 || true)"
    if [ -n "$ROOT_DEVICE" ] && [ -e "/dev/$ROOT_DEVICE" ]; then
      LSBLK_TARGET="/dev/$ROOT_DEVICE"
    else
      LSBLK_TARGET="$SOURCE_DEVICE"
    fi
  fi
fi
if [ -n "$LSBLK_TARGET" ]; then
  LSBLK_JSON="$(lsblk -J -b -o NAME,KNAME,PKNAME,PATH,TYPE,SIZE,FSTYPE,MOUNTPOINT "$LSBLK_TARGET" 2>/dev/null | tr '\n' ' ' || true)"
else
  LSBLK_JSON="$(lsblk -J -b -o NAME,KNAME,PKNAME,PATH,TYPE,SIZE,FSTYPE,MOUNTPOINT 2>/dev/null | tr '\n' ' ' || true)"
fi
...
`;
    ...
    const probe = await ssh.execScript(probeScript, { sudo: true, timeoutMs: 90000 });
    const out = String(probe.stdout || '');
    const preferred = readTaggedSection(out, '__PREFERRED__', '__RESOLVED__') || preferredPath;
    const resolved = readTaggedSection(out, '__RESOLVED__', '__PATH_EXISTS__') || preferred;
    const pathExists = readTaggedSection(out, '__PATH_EXISTS__', '__CANDIDATES__') === 'true';
    const candidates = readTaggedSection(out, '__CANDIDATES__', '__FINDMNT__')
      .split('\n')
      .map((item) => String(item || '').trim())
      .filter(Boolean);
    const findmnt = safeJsonParse(readTaggedSection(out, '__FINDMNT__', '__DF__'), null);
    const df = parseDfLine(readTaggedSection(out, '__DF__', '__LSBLK__'));
    const lsblk = safeJsonParse(readTaggedSection(out, '__LSBLK__'), { blockdevices: [] });

    return buildStorageDevicesSummary({
  ...
  probePromise = (async () => {
    try {
      ...
      if (!injectedSsh && storageDevicesCache.promise === probePromise) {
        storageDevicesCache = {
          ...storageDevicesCache,
          promise: null,
        };
  ...
  if (!injectedSsh) {
    storageDevicesCache = { ...storageDevicesCache, key: cacheKey, promise: probePromise };
  }
