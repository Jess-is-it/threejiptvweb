import 'server-only';

import { Client } from 'ssh2';
import { pipeline } from 'node:stream/promises';

function once(emitter, event) {
  return new Promise((resolve) => emitter.once(event, resolve));
}

function safeTrim(s, max = 10_000_000) {
  const str = String(s || '');
  if (str.length <= max) return str;
  return `${str.slice(0, max)}\n…(truncated ${str.length - max} chars)`;
}

const DEFAULT_CAPTURE_LIMIT = 2_000_000;

function createOutputCollector({ limit = DEFAULT_CAPTURE_LIMIT, mode = 'head' } = {}) {
  const maxBytes = Math.max(0, Math.floor(Number(limit) || 0));
  const captureMode = String(mode || 'head').trim().toLowerCase() === 'tail' ? 'tail' : 'head';
  const chunks = [];
  let bufferedBytes = 0;
  let truncatedBytes = 0;

  const toBuffer = (chunk) => {
    if (Buffer.isBuffer(chunk)) return chunk;
    if (chunk instanceof Uint8Array) return Buffer.from(chunk.buffer, chunk.byteOffset, chunk.byteLength);
    return Buffer.from(String(chunk || ''), 'utf8');
  };

  return {
    push(chunk) {
      if (maxBytes <= 0 || chunk === null || chunk === undefined) return;
      const buf = toBuffer(chunk);
      if (!buf.length) return;

      if (captureMode === 'tail') {
        const capture = buf.length > maxBytes ? Buffer.from(buf.subarray(buf.length - maxBytes)) : buf;
        if (capture.length !== buf.length) truncatedBytes += buf.length - capture.length;
        chunks.push(capture);
        bufferedBytes += capture.length;
        while (bufferedBytes > maxBytes && chunks.length) {
          const over = bufferedBytes - maxBytes;
          const first = chunks[0];
          if (first.length <= over) {
            chunks.shift();
            bufferedBytes -= first.length;
            truncatedBytes += first.length;
            continue;
          }
          chunks[0] = first.subarray(over);
          bufferedBytes -= over;
          truncatedBytes += over;
        }
        return;
      }

      if (bufferedBytes >= maxBytes) {
        truncatedBytes += buf.length;
        return;
      }

      const remaining = maxBytes - bufferedBytes;
      if (buf.length <= remaining) {
        chunks.push(buf);
        bufferedBytes += buf.length;
        return;
      }

      chunks.push(Buffer.from(buf.subarray(0, remaining)));
      bufferedBytes += remaining;
      truncatedBytes += buf.length - remaining;
    },
    text() {
      if (!chunks.length || bufferedBytes <= 0) return '';
      const value = Buffer.concat(chunks, bufferedBytes).toString('utf8');
      return safeTrim(value, maxBytes || undefined);
    },
    wasTruncated() {
      return truncatedBytes > 0;
    },
  };
}

export class SSHService {
  /**
   * @param {object} opts
   * @param {string} opts.host
   * @param {number} [opts.port]
   * @param {string} opts.username
   * @param {'password'|'privateKey'} opts.authType
   * @param {string} [opts.password]
   * @param {string} [opts.privateKey]
   * @param {string} [opts.passphrase]
   * @param {string} [opts.sudoPassword]
   */
  constructor(opts) {
    this.opts = opts || {};
    this.client = new Client();
    this.connected = false;
    this._sftp = null;
  }

  async connect({ timeoutMs = 15000 } = {}) {
    if (this.connected) return;

    const { host, port = 22, username, authType } = this.opts;
    if (!host || !username || !authType) throw new Error('SSH config is incomplete.');

    const connectOpts = {
      host,
      port: Number(port) || 22,
      username,
      readyTimeout: timeoutMs,
      keepaliveInterval: 15000,
      keepaliveCountMax: 2,
    };

    if (authType === 'password') {
      connectOpts.password = String(this.opts.password || '');
      if (!connectOpts.password) throw new Error('SSH password is missing.');
    } else if (authType === 'privateKey') {
      connectOpts.privateKey = String(this.opts.privateKey || '');
      if (!connectOpts.privateKey) throw new Error('SSH private key is missing.');
      if (this.opts.passphrase) connectOpts.passphrase = String(this.opts.passphrase);
    } else {
      throw new Error('Unsupported SSH authType.');
    }

    this.client.connect(connectOpts);
    const err = await Promise.race([
      once(this.client, 'ready').then(() => null),
      once(this.client, 'error'),
    ]);
    if (err) throw err;
    this.connected = true;
  }

  async close() {
    try {
      if (this._sftp && typeof this._sftp.end === 'function') {
        try {
          this._sftp.end();
        } catch {}
      }
      this.client.end();
    } catch {}
    this.connected = false;
    this._sftp = null;
  }

  async sftp() {
    await this.connect();
    if (this._sftp) return this._sftp;
    this._sftp = await new Promise((resolve, reject) => {
      this.client.sftp((err, sftp) => {
        if (err) return reject(err);
        resolve(sftp);
      });
    });
    return this._sftp;
  }

  async uploadStream(stream, remotePath, { mode = null } = {}) {
    const p = String(remotePath || '').trim();
    if (!p) throw new Error('remotePath is required.');
    const sftp = await this.sftp();
    const opts = mode ? { mode } : undefined;
    const writeStream = sftp.createWriteStream(p, opts);
    await pipeline(stream, writeStream);
    return true;
  }

  async exec(command, { timeoutMs = 60000, sudo = false } = {}) {
    await this.connect();
    const cmd = sudo ? this.#sudoWrap(command) : command;

    let streamRef = null;
    const p = new Promise((resolve, reject) => {
      this.client.exec(cmd, { env: { LANG: 'C', LC_ALL: 'C' } }, (err, stream) => {
        if (err) return reject(err);
        streamRef = stream;

        const stdout = createOutputCollector({ mode: 'head' });
        const stderr = createOutputCollector({ mode: 'head' });
        const done = (code, signal) =>
          resolve({
            code: Number.isFinite(code) ? code : null,
            signal: signal || null,
            stdout: stdout.text(),
            stderr: stderr.text(),
            stdoutTruncated: stdout.wasTruncated(),
            stderrTruncated: stderr.wasTruncated(),
          });

        stream.on('data', (d) => stdout.push(d));
        stream.stderr.on('data', (d) => stderr.push(d));
        stream.on('error', reject);
        stream.on('close', done);

        if (sudo) {
          const sudoPass = String(this.opts.sudoPassword || '');
          if (sudoPass) {
            // Feed sudo password once.
            try {
              stream.write(`${sudoPass}\n`);
            } catch {}
          }
        }
      });
    });

    if (!timeoutMs) return p;
    let t;
    const timeout = new Promise((_resolve, reject) => {
      t = setTimeout(() => {
        try {
          streamRef?.close();
        } catch {}
        reject(new Error(`SSH command timed out after ${timeoutMs}ms`));
      }, timeoutMs);
    });
    return Promise.race([p, timeout]).finally(() => clearTimeout(t));
  }

  async execScript(script, { timeoutMs = 120000, sudo = false, args = [] } = {}) {
    await this.connect();
    const argStr = Array.isArray(args) ? args.map((a) => String(a)).join(' ') : '';
    const cmd = sudo ? this.#sudoWrap(`bash -s -- ${argStr}`) : `bash -s -- ${argStr}`;

    let streamRef = null;
    const p = new Promise((resolve, reject) => {
      this.client.exec(cmd, { env: { LANG: 'C', LC_ALL: 'C' } }, (err, stream) => {
        if (err) return reject(err);
        streamRef = stream;

        const stdout = createOutputCollector({ mode: 'tail' });
        const stderr = createOutputCollector({ mode: 'tail' });

        stream.on('data', (d) => stdout.push(d));
        stream.stderr.on('data', (d) => stderr.push(d));
        stream.on('error', reject);
        stream.on('close', (code, signal) =>
          resolve({
            code: Number.isFinite(code) ? code : null,
            signal: signal || null,
            stdout: stdout.text(),
            stderr: stderr.text(),
            stdoutTruncated: stdout.wasTruncated(),
            stderrTruncated: stderr.wasTruncated(),
          })
        );

        if (sudo) {
          const sudoPass = String(this.opts.sudoPassword || '');
          if (sudoPass) {
            try {
              stream.write(`${sudoPass}\n`);
            } catch {}
          }
        }

        stream.end(`${String(script || '')}\n`);
      });
    });

    if (!timeoutMs) return p;
    let t;
    const timeout = new Promise((_resolve, reject) => {
      t = setTimeout(() => {
        try {
          streamRef?.close();
        } catch {}
        reject(new Error(`SSH script timed out after ${timeoutMs}ms`));
      }, timeoutMs);
    });
    return Promise.race([p, timeout]).finally(() => clearTimeout(t));
  }

  #sudoWrap(cmd) {
    // -S: read password from stdin (we feed it)
    // -p '': avoid prompt text in stderr
    const script = String(cmd || '');
    // Use single-quote shell escaping so `$VAR` is expanded by the target shell,
    // not by an intermediate shell layer before command execution.
    const quoted = `'${script.replace(/'/g, `'\"'\"'`)}'`;
    return `sudo -S -p '' bash -lc ${quoted}`;
  }
}
