...
function providerSupportsRequestedType(providerKey = '', type = 'movie') {
  ...
  const mediaType = String(type || 'movie').trim().toLowerCase();
  if (mediaType === 'series' && key === 'yts') return false;
  return true;
}
...
export async function getDownloadSourcesState() {
  ...
  const providers = sortProvidersForRun(db).map((p) => providerPublicView(db, p));
...
export async function updateDownloadSourceProvider({ providerId, patch = null, order = null } = {}) {
  ...
  if (Array.isArray(order) && order.length) {
    ...
    for (const p of db.sourceProviders) {
      ...
      if (orderMap.has(key)) {
        p.priority = orderMap.get(key);
        p.updatedAt = now();
        changed = true;
      }
    }
    db.sourceProviders = normalizePriorities(db.sourceProviders);
  }
  ...
  if (providerId && patch && typeof patch === 'object') {
    ...
    if (patch.enabled !== undefined) p.enabled = Boolean(patch.enabled);
    if (patch.priority !== undefined) p.priority = clamp(patch.priority, { min: 1, max: 9999, fallback: p.priority || 1 });
  ...
  await persistIfNeeded(db, changed);
  return getDownloadSourcesState();
}
...
async function runProviderActionWithRotation(db, provider, opts = {}) {
  ...
  if (!force && !includeBlocked && isBackoffActive(provider)) {
    const logRow = appendLog(db, {
      ...
      correlationId,
      jobId,
      detailsJson: { backoffUntil: provider.backoffUntil },
    });
    return {
      ok: false,
      skipped: true,
  ...
  if (!candidates.length) {
    applyFailure(provider, ERROR_CATEGORY.BLOCKED, { forceBlock: true });
  ...
  applyFailure(provider, lastErr?.category || ERROR_CATEGORY.BLOCKED, { forceBlock: true });
  appendLog(db, {
    providerId,
    providerKey: provider.key,
    action: LOG_ACTION.BACKOFF,
...
export async function testDownloadSourceProvider({ providerId, query = '', force = false, onlyDomain = '', correlationId = '', jobId = '' } = {}) {
  const { db, changed: initialChanged } = await loadMutableDb();
  let changed = initialChanged;
...
export async function testAllDownloadSources({ force = false } = {}) {
  const { db, changed: initialChanged } = await loadMutableDb();
  let changed = initialChanged;

  const results = [];
  for (const provider of sortProvidersForRun(db)) {
  ...
  return {
    ok: true,
    results,
    providers: sortProvidersForRun(db).map((p) => providerPublicView(db, p)),
  };
}
...
export async function clearDownloadSourceLogs({ provider = 'all' } = {}) {
  const { db, changed: initialChanged } = await loadMutableDb();
  let changed = initialChanged;
  ...
  const nextLogs =
    providerId === 'all'
      ? []
      : beforeLogs.filter((x) => String(x?.providerId || '').toLowerCase() !== providerId);
  const deletedCount = Math.max(0, beforeLogs.length - nextLogs.length);
  if (deletedCount > 0 || providerId === 'all') {
...
export async function searchBestDownloadSource({
  ...
  const providers = sortProvidersForRun(db).filter((p) => p.enabled !== false && providerSupportsRequestedType(p.key, type));
  const attempts = [];
  const collected = [];
