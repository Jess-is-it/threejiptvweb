...
export function sourceMatchesRequestedMedia(source, { title, year = null, type = 'movie', imdbId = '' } = {}) {
  ...
  // EZTV (series) is reliably scoped by IMDb id, but torrent names often don't match the canonical TMDB title/year.
  // Allow an IMDb-id match when provided.
  if (mediaType === 'series' && String(source?.provider || '').trim().toLowerCase() === 'eztv') {
    const wanted = String(imdbId || '').trim();
    const got = String(source?.imdbIdDigits || '').trim();
    const m1 = wanted.match(/\btt(\d{7,8})\b/i);
    const wantedDigits = m1?.[1] || (wanted.match(/\b(\d{7,8})\b/)?.[1] || '');
    const gotDigits = got.match(/\b(\d{7,8})\b/)?.[1] || '';
