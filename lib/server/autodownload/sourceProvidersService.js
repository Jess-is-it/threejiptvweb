...
const DEFAULT_SERIES_TEST_QUERY = 'tt0944947';
...
function extractImdbDigitsForMatch(value) {
  const raw = String(value || '').trim();
  if (!raw) return '';
  const imdbMatch = raw.match(/\btt(\d{7,8})\b/i);
  if (imdbMatch?.[1]) return imdbMatch[1];
  const digitsMatch = raw.match(/\b(\d{7,8})\b/);
...
export function sourceMatchesRequestedMedia(source, { title, year = null, type = 'movie', imdbId = '' } = {}) {
  const mediaType = String(type || 'movie').trim().toLowerCase();
  if (!providerSupportsRequestedType(source?.provider || '', mediaType)) return false;
  if (mediaType === 'series') {
    const wantedDigits = extractImdbDigitsForMatch(imdbId);
    const gotDigits = extractImdbDigitsForMatch(source?.imdbIdDigits);
    if (wantedDigits && gotDigits && wantedDigits === gotDigits) return true;
  }
...
function parseSeasonEpisodeFromName(name) {
  ...
  if (!s) return null;
  // Support common patterns: S01E02, s1e2, 1x02
...
function filterResultsForEpisodeHint(results, hint) {
  if (!hint || typeof hint !== 'object') return Array.isArray(results) ? results : [];
  const wantedSeason = Number(hint.season || 0);
  const wantedEpisode = Number(hint.episode || 0);
  if (!Number.isFinite(wantedSeason) || wantedSeason <= 0) return Array.isArray(results) ? results : [];
  if (!Number.isFinite(wantedEpisode) || wantedEpisode <= 0) return Array.isArray(results) ? results : [];

  return (Array.isArray(results) ? results : []).filter((row) => {
    const tag = parseSeasonEpisodeFromName(row?.name || row?.title || '');
    if (!tag) return false;
    return tag.season === wantedSeason && tag.episode === wantedEpisode;
  });
}
...
function normalizeJackettEndpoint(input) {
  const raw = String(input || DEFAULT_JACKETT_ENDPOINT).trim() || DEFAULT_JACKETT_ENDPOINT;
  return raw.startsWith('/') ? raw : `/${raw}`;
}

function normalizeProwlarrEndpoint(input) {
  const raw = String(input || DEFAULT_PROWLARR_ENDPOINT).trim() || DEFAULT_PROWLARR_ENDPOINT;
  return raw.startsWith('/') ? raw : `/${raw}`;
}
...
function defaultProviders() {
  ...
  return [
    ...
    },
    {
      ...
      backoffReasonCategory: null,
      backoffReasonMessage: '',
      activeDomain: DEFAULT_JACKETT_DOMAINS[0],
      config: {
        endpoint: DEFAULT_JACKETT_ENDPOINT,
        apiKey: '',
      },
      createdAt: t,
      updatedAt: t,
    },
    {
      ...
      backoffReasonCategory: null,
      backoffReasonMessage: '',
      activeDomain: DEFAULT_PROWLARR_DOMAINS[0],
      config: {
        endpoint: DEFAULT_PROWLARR_ENDPOINT,
        apiKey: '',
      },
      createdAt: t,
      updatedAt: t,
...
function normalizeProvider(raw, defaults) {
  ...
  if (key === 'yts') {
    ...
    const cfgRaw = base.config && typeof base.config === 'object' ? base.config : {};
    const domains = normalizeDomains(cfgRaw.domains || DEFAULT_JACKETT_DOMAINS);
    common.config = {
      domains: domains.length ? domains : DEFAULT_JACKETT_DOMAINS,
      endpoint: normalizeJackettEndpoint(cfgRaw.endpoint || defaults.config.endpoint || DEFAULT_JACKETT_ENDPOINT),
      apiKey: String(cfgRaw.apiKey || defaults.config.apiKey || '').trim(),
    };
    if (!common.activeDomain) {
      common.activeDomain = common.config.domains[0] || DEFAULT_JACKETT_DOMAINS[0];
    }
  } else if (key === 'prowlarr') {
    const cfgRaw = base.config && typeof base.config === 'object' ? base.config : {};
    const domains = normalizeDomains(cfgRaw.domains || DEFAULT_PROWLARR_DOMAINS);
    common.config = {
      domains: domains.length ? domains : DEFAULT_PROWLARR_DOMAINS,
      endpoint: normalizeProwlarrEndpoint(cfgRaw.endpoint || defaults.config.endpoint || DEFAULT_PROWLARR_ENDPOINT),
      apiKey: String(cfgRaw.apiKey || defaults.config.apiKey || '').trim(),
    };
    if (!common.activeDomain) {
      common.activeDomain = common.config.domains[0] || DEFAULT_PROWLARR_DOMAINS[0];
    }
  } else {
...
function defaultTestQueryForType(provider, type = 'movie') {
  const mediaType = normalizeMediaType(type);
  if (mediaType === 'series') return DEFAULT_SERIES_TEST_QUERY;
  return String(provider?.testQuery || DEFAULT_TEST_QUERY).trim() || DEFAULT_TEST_QUERY;
}
...
function defaultDomainsForProvider(providerKey) {
  ...
  if (key === 'jackett') return [...DEFAULT_JACKETT_DOMAINS];
  if (key === 'prowlarr') return [...DEFAULT_PROWLARR_DOMAINS];
...
function retryWindowExpired({ backoffUntil = null, updatedAt = null, lastCheckedAt = null, backoffMinutes = 30 } = {}, at = now()) {
  const until = Number(backoffUntil || 0) || 0;
  if (until > 0) return until <= at;
  const last = Number(updatedAt || lastCheckedAt || 0) || 0;
  if (last <= 0) return false;
  return last + clamp(backoffMinutes, { min: 1, max: 1440, fallback: 30 }) * 60 * 1000 <= at;
}

function clearExpiredProviderBackoff(provider, at = now()) {
  if (!provider || typeof provider !== 'object') return false;
  const status = String(provider?.status || '').toLowerCase();
  const expired = retryWindowExpired(
    {
      backoffUntil: provider.backoffUntil,
      updatedAt: provider.updatedAt,
      lastCheckedAt: provider.lastCheckedAt,
      backoffMinutes: provider.backoffMinutesOnError,
    },
    at
  );
  if (!expired && status !== PROVIDER_STATUS.BLOCKED) return false;
  ...
  if (status === PROVIDER_STATUS.BLOCKED) {
    provider.status = provider.lastSuccessAt ? PROVIDER_STATUS.HEALTHY : provider.lastErrorCategory ? PROVIDER_STATUS.DEGRADED : PROVIDER_STATUS.UNKNOWN;
  }
  provider.updatedAt = at;
  return true;
}

function clearExpiredDomainBackoff(domainRow, provider, at = now()) {
  if (!domainRow || typeof domainRow !== 'object') return false;
  const status = String(domainRow?.status || '').toLowerCase();
  const expired = retryWindowExpired(
    {
      backoffUntil: domainRow.backoffUntil,
      updatedAt: domainRow.updatedAt,
      lastCheckedAt: domainRow.lastCheckedAt,
      backoffMinutes: provider?.backoffMinutesOnError,
    },
    at
  );
  if (!expired && status !== DOMAIN_STATUS.BLOCKED) return false;
  if (!expired) return false;

  domainRow.backoffUntil = null;
  domainRow.failureStreak = 0;
  if (status === DOMAIN_STATUS.BLOCKED) {
    domainRow.status = domainRow.lastSuccessAt ? DOMAIN_STATUS.HEALTHY : domainRow.lastErrorCategory ? DOMAIN_STATUS.DEGRADED : DOMAIN_STATUS.UNKNOWN;
  }
  domainRow.updatedAt = at;
  return true;
}
...
function syncProviderDomains(db, provider, domainsInput = null, opts = {}) {
  ...
  if (key === 'jackett') {
    provider.config.endpoint = normalizeJackettEndpoint(provider.config.endpoint || DEFAULT_JACKETT_ENDPOINT);
    provider.config.apiKey = String(provider.config.apiKey || '').trim();
  }
  if (key === 'prowlarr') {
    provider.config.endpoint = normalizeProwlarrEndpoint(provider.config.endpoint || DEFAULT_PROWLARR_ENDPOINT);
    provider.config.apiKey = String(provider.config.apiKey || '').trim();
  }
  if (key === 'eztv') {
...
function ensureStore(db) {
  ...
  for (const p of db.sourceProviders) {
    ...
    const staleUnsupportedAdapter =
      (p.key === 'jackett' || p.key === 'prowlarr') &&
      /unsupported provider adapter/i.test(String(p?.lastErrorMessage || ''));
    if (staleUnsupportedAdapter) {
      p.status = PROVIDER_STATUS.UNKNOWN;
      p.failureStreak = 0;
      p.backoffUntil = null;
      p.backoffReasonCategory = null;
      p.backoffReasonMessage = '';
      p.lastErrorCategory = null;
      p.lastErrorMessage = '';
      p.updatedAt = now();
      changed = true;
    }

    if (clearExpiredProviderBackoff(p)) {
      changed = true;
    }
  }

  for (const row of db.sourceProviderDomains) {
    const providerKey = String(row?.providerKey || row?.providerId || '').trim().toLowerCase();
    const provider = db.sourceProviders.find((p) => String(p?.key || p?.id || '').toLowerCase() === providerKey) || null;
    if (clearExpiredDomainBackoff(row, provider)) {
      changed = true;
    }

    const staleUnsupportedAdapterDomain =
      (providerKey === 'jackett' || providerKey === 'prowlarr') &&
      /unsupported provider adapter/i.test(String(row?.lastErrorMessage || ''));
    if (!staleUnsupportedAdapterDomain) continue;
...
function deriveStatus(provider, logs, domainRows = [], type = null) {
  const at = now();
  if (type ? !providerEnabledForType(provider, type) : provider?.enabled === false) return PROVIDER_STATUS.DISABLED;
  ...
  const blockedWindow = (Array.isArray(logs) ? logs : []).filter(
    (x) =>
      x?.success === false &&
      String(x?.action || '').toLowerCase() !== LOG_ACTION.BACKOFF &&
      x?.errorCategory === ERROR_CATEGORY.BLOCKED &&
      Number(x?.timestamp || 0) >= at - 2 * 60 * 60 * 1000
  );
  ...
  const activeDomain = normalizeDomain(provider?.activeDomain || '');
  const activeDomainRow = activeDomain ? domains.find((x) => normalizeDomain(x?.domain || '') === activeDomain) : null;
  if (activeDomainRow?.status === DOMAIN_STATUS.DEGRADED || (!activeDomainRow && domains.some((x) => x?.status === DOMAIN_STATUS.DEGRADED))) {
...
function appendLog(db, entry) {
  ...
  const row = {
    ...
    selectionLogId: entry?.selectionLogId ? String(entry.selectionLogId).trim() : '',
    query: entry?.query ? String(entry.query).trim().slice(0, 240) : '',
...
function providerPublicView(db, provider, { type = null } = {}) {
  ...
  const status = deriveStatus(provider, logs, domainRows, type);
...
export async function getDownloadSourcesState({ type = 'movie' } = {}) {
  ...
  let providers = sortProvidersForRun(db, t).map((p) => {
    const view = providerPublicView(db, p, { type: t });
...
async function runProviderActionWithRotation(db, provider, opts = {}) {
  ...
  const selectionLogId = String(opts.selectionLogId || '').trim();
  const query = String(opts.query || defaultTestQueryForType(provider, opts.type || 'movie')).trim() || defaultTestQueryForType(provider, opts.type || 'movie');
  const mediaType = normalizeMediaType(opts.type || 'movie');
  const episodeHint = opts?.episodeHint && typeof opts.episodeHint === 'object' ? opts.episodeHint : null;
  ...
  for (const domainRow of candidates) {
    ...
    try {
      ...
      if (mode === 'search' && mediaType === 'series' && episodeHint) {
        // Apply episode hint filtering at the provider-action layer so provider logs reflect
        // what the selection engine actually consumes.
        res.results = filterResultsForEpisodeHint(res?.results || [], episodeHint);
      }
...
async function runSingleProviderTest(db, provider, opts = {}) {
  ...
  return runProviderActionWithRotation(db, provider, {
    mode: 'test',
    query: opts.query || defaultTestQueryForType(provider, opts.type || 'movie'),
...
export async function testDownloadSourceProvider({ providerId, query = '', type = 'movie', force = false, onlyDomain = '', correlationId = '', jobId = '' } = {}) {
  ...
  return {
    ...result,
    provider: providerPublicView(db, provider, { type }),
  };
}
...
export async function testAllDownloadSources({ force = false, type = 'movie' } = {}) {
  ...
  for (const provider of sortProvidersForRun(db, mediaType).filter(
    ...
    const r = await runSingleProviderTest(db, provider, {
      type: mediaType,
      query: defaultTestQueryForType(provider, mediaType),
      force,
  ...
  return {
    ok: true,
    results,
    providers: sortProvidersForRun(db, mediaType).map((p) => providerPublicView(db, p, { type: mediaType })),
  };
}
...
export async function validateDownloadSourceProviderDomains({
  ...
  const mediaType = normalizeMediaType(type);
  const testQuery = String(query || defaultTestQueryForType(provider, mediaType)).trim() || defaultTestQueryForType(provider, mediaType);
...
export async function queryDownloadSourceLogs({
  ...
  const rowOffset = clamp(Number(offset || 0), { min: 0, max: 50_000_000, fallback: 0 });
  const providerId = String(provider || 'all').trim().toLowerCase();
  const selectionLogFilter = String(selectionLogId || '').trim();
  ...
  if (selectionLogFilter) {
    rows = rows.filter((x) => String(x?.selectionLogId || '').trim() === selectionLogFilter);
  } else {
  ...
  if (selectionLogFilter) {
    ...
    for (const row of rows) {
      ...
      if (String(row?.action || '').toLowerCase() === String(LOG_ACTION.BACKOFF || 'backoff').toLowerCase()) backoffCount += 1;
      ...
      if (!query) query = String(row?.query || '').trim();
    }

    selectionLogSummary = {
      selectionLogId: selectionLogFilter,
      ...
      blockedCount,
      totalResults,
      avgDurationMs: durationSamples ? durationTotal / durationSamples : 0,
      firstTimestamp,
      latestTimestamp,
  ...
  rows = rows.slice(rowOffset, rowOffset + maxRows);
...
export async function clearDownloadSourceLogs({ provider = 'all', type = 'movie', selectionLogId = '' } = {}) {
  const { db, changed: initialChanged } = await loadMutableDb();
  let changed = initialChanged;
  ...
  const nextLogs = beforeLogs.filter((x) => {
    ...
    const selectionMatches = selectionLogFilter ? String(x?.selectionLogId || '').trim() === selectionLogFilter : true;
    if (!providerMatches || !selectionMatches) return true;
