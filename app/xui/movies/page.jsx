...
export default function XuiMovies() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const url = `/api/xuione/vod?streamBase=${encodeURIComponent(session.streamBase)}`;
        const r = await fetch(url);
        const data = await readJsonSafe(r);
        if (!alive) return;
        if (!r.ok || !data.ok) throw new Error(data?.error || `HTTP ${r.status}`);
        setItems((data.items || []).slice(0, 60).map(m => ({
          ...m,
  ...
  return (
    <Protected>
      <section className="py-6">
        <h1 className="mb-4 text-2xl font-bold">Your Library — Movies (Xuione)</h1>
        {!session?.streamBase ? (
        ...
        <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-5 lg:grid-cols-7 xl:grid-cols-9">
          {showSkeleton
            ...
            : items.map((it) => <MediaCard key={it.id} item={it} kind="movie" />)}
        </div>
      </section>
    </Protected>
  );
