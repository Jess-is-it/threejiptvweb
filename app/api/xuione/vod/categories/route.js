...
async function handle(req) {
  try {
    ...
    const raw = await xtreamWithFallback({ server, username, password, action: 'get_vod_categories' });
    ...
    return NextResponse.json({ ok: false, error: e?.message || 'VOD categories error' }, { status: 502 });
  }
}
